
Partial Class rpt_Estoque_Distribuidor_Sugestao
    Inherits System.Web.UI.Page
    Dim M As New clsMaster
    Dim Pagina As String = "Estoque"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim sql_cmb_dst As String

        'CONTROLA OS ACESSO E FUNÇÕES CONFORME O PERFIL DE ACESSO
        If Session("COD_PERFIL_LOGIN") = "3" Then Alert("Você não tem acesso a esta página", True, "Default.aspx")
        If Session("COD_PERFIL_LOGIN") = "4" Then Alert("Você não tem acesso a esta página", True, "Default.aspx")
        If Session("COD_PERFIL_LOGIN") = "5" Then Alert("Você não tem acesso a esta página", True, "Default.aspx")
        If Session("COD_PERFIL_LOGIN") = "6" Then Alert("Você não tem acesso a esta página", True, "Default.aspx")

        'GRAVA LOG DE ACESSO A PAGINA
        If Session("PAGINA_ATUAL") <> HttpContext.Current.Handler.ToString Then
            M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "ACESSOU", "")
            Session("PAGINA_ATUAL") = HttpContext.Current.Handler.ToString
        End If

        'DEFINE SQL DO COMBO DISTRIBUIDOR
        sql_cmb_dst = "SELECT '# Selecione' as [GRUPO] UNION ALL SELECT DISTINCT [GRUPO] FROM [VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_PRODUTO_TOTAL] WHERE (([ANO] = " & Year(Now()) & ")) ORDER BY [GRUPO]"
        dts_DISTRIBUIDORES.SelectCommand = sql_cmb_dst
        dts_DISTRIBUIDORES.DataBind()

    End Sub

    Public Sub Atualiza_Relatorio()
        Dim cnn As New System.Data.SqlClient.SqlConnection
        Dim cmd As New System.Data.SqlClient.SqlCommand
        Dim dtr As System.Data.SqlClient.SqlDataReader
        Dim sql As String

        Dim cnn_Dst As New System.Data.SqlClient.SqlConnection
        Dim cmd_Dst As New System.Data.SqlClient.SqlCommand
        Dim dtr_Dst As System.Data.SqlClient.SqlDataReader
        Dim sql_Dst As String

        Dim cnn_Linha As New System.Data.SqlClient.SqlConnection
        Dim cmd_Linha As New System.Data.SqlClient.SqlCommand
        Dim dtr_Linha As System.Data.SqlClient.SqlDataReader
        Dim sql_Linha As String

        Dim cnn_ESTABELECIMENTOS As New System.Data.SqlClient.SqlConnection
        Dim cmd_ESTABELECIMENTOS As New System.Data.SqlClient.SqlCommand

        Dim LINHA_PRODUTO As String
        Dim GRUPO_DISTRIBUIDOR As String
        Dim PRODUTO As String
        Dim COD_PRODUTO As String
        Dim X As Integer
        'Dim Y As Integer
        Dim ANO As Integer
        Dim rw As TableRow
        Dim cel As TableCell

        Dim JAN_ESTOQUE As Integer
        Dim FEV_ESTOQUE As Integer
        Dim MAR_ESTOQUE As Integer
        Dim ABR_ESTOQUE As Integer
        Dim MAI_ESTOQUE As Integer
        Dim JUN_ESTOQUE As Integer
        Dim JUL_ESTOQUE As Integer
        Dim AGO_ESTOQUE As Integer
        Dim SET_ESTOQUE As Integer
        Dim OUT_ESTOQUE As Integer
        Dim NOV_ESTOQUE As Integer
        Dim DEZ_ESTOQUE As Integer
        Dim TOTAL_ESTOQUE As Integer
        Dim ESTOQUE_INICIAL As Integer

        Dim JAN_VENDA As Integer
        Dim FEV_VENDA As Integer
        Dim MAR_VENDA As Integer
        Dim ABR_VENDA As Integer
        Dim MAI_VENDA As Integer
        Dim JUN_VENDA As Integer
        Dim JUL_VENDA As Integer
        Dim AGO_VENDA As Integer
        Dim SET_VENDA As Integer
        Dim OUT_VENDA As Integer
        Dim NOV_VENDA As Integer
        Dim DEZ_VENDA As Integer
        Dim TOTAL_VENDA As Integer
        Dim YTD_VENDA As Integer
        Dim YTD_VENDA_MEDIA As Double
        Dim MES_ATUAL_VENDA As Integer


        Dim JAN_DEMANDA As Integer
        Dim FEV_DEMANDA As Integer
        Dim MAR_DEMANDA As Integer
        Dim ABR_DEMANDA As Integer
        Dim MAI_DEMANDA As Integer
        Dim JUN_DEMANDA As Integer
        Dim JUL_DEMANDA As Integer
        Dim AGO_DEMANDA As Integer
        Dim SET_DEMANDA As Integer
        Dim OUT_DEMANDA As Integer
        Dim NOV_DEMANDA As Integer
        Dim DEZ_DEMANDA As Integer
        Dim TOTAL_DEMANDA As Integer
        Dim YTD_DEMANDA As Integer
        Dim YTD_DEMANDA_MEDIA As Double
        Dim MES_ATUAL_DEMANDA As Integer

        'RECUPERA ANO INICIAL E LINHA DE PRODUTOS
        'LINHA_PRODUTO = cmb_LINHA.Text
        GRUPO_DISTRIBUIDOR = cmb_DISTRIBUIDOR.Text
        'ANO = Year(Now())
        Integer.TryParse(Year(Now()), ANO)

        'ABRE CONEXÕES
        cnn.ConnectionString = ConfigurationManager.ConnectionStrings("cnnSTR").ConnectionString
        cnn.Open()
        cnn_Dst.ConnectionString = ConfigurationManager.ConnectionStrings("cnnSTR").ConnectionString
        cnn_Dst.Open()
        cnn_Linha.ConnectionString = ConfigurationManager.ConnectionStrings("cnnSTR").ConnectionString
        cnn_Linha.Open()

        LINHA_PRODUTO = ""
        'Recupera LINHAS
        sql_Linha = "SELECT DISTINCT LINHA FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_PRODUTO_TOTAL WHERE ANO = " & ANO & " AND GRUPO = '" & GRUPO_DISTRIBUIDOR & "' ORDER BY LINHA "
        cmd_Linha.Connection = cnn_Linha
        cmd_Linha.CommandText = sql_Linha
        dtr_Linha = cmd_Linha.ExecuteReader

        Do While dtr_Linha.Read
            If dtr_Linha("LINHA") <> "ACESSORIOS GEMSTAR" And dtr_Linha("LINHA") <> "ACESSORIOS EDDS" And dtr_Linha("LINHA") <> "RECEPTAL" Then
                LINHA_PRODUTO = dtr_Linha("LINHA")
                'Recupera PRODUTOS
                sql_Dst = "SELECT DISTINCT PRODUTO, COD_PRODUTO FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_PRODUTO_TOTAL WHERE ANO = " & ANO & " and LINHA = '" & LINHA_PRODUTO & "' AND GRUPO = '" & GRUPO_DISTRIBUIDOR & "' ORDER BY PRODUTO "
                cmd_Dst.Connection = cnn_Dst
                cmd_Dst.CommandText = sql_Dst
                dtr_Dst = cmd_Dst.ExecuteReader

                'LINHA CABECALHO
                rw = New TableRow
                tbl_Report.Rows.Add(rw)

                'COLUNA NOME DO LINHA
                cel = New TableCell
                rw.Cells.Add(cel)
                cel.Text = LINHA_PRODUTO
                cel.Font.Bold = True
                cel.HorizontalAlign = HorizontalAlign.Left
                cel.BackColor = Drawing.Color.Gainsboro

                'COLUNA YTD
                cel = New TableCell
                rw.Cells.Add(cel)
                cel.Text = "Venda YTD"
                cel.Font.Bold = False
                cel.HorizontalAlign = HorizontalAlign.Center
                cel.BackColor = Drawing.Color.Gainsboro

                'COLUNA TOTAL
                cel = New TableCell
                rw.Cells.Add(cel)
                cel.Text = "Venda Mês Atual"
                cel.Font.Bold = False
                cel.HorizontalAlign = HorizontalAlign.Center
                cel.BackColor = Drawing.Color.Gainsboro

                'COLUNA YTD
                cel = New TableCell
                rw.Cells.Add(cel)
                cel.Text = "Venda YTD Média"
                cel.Font.Bold = False
                cel.HorizontalAlign = HorizontalAlign.Center
                cel.BackColor = Drawing.Color.Gainsboro

                'COLUNA YTD
                cel = New TableCell
                rw.Cells.Add(cel)
                cel.Text = "Demanda YTD"
                cel.Font.Bold = False
                cel.HorizontalAlign = HorizontalAlign.Center
                cel.BackColor = Drawing.Color.Gainsboro

                'COLUNA TOTAL
                cel = New TableCell
                rw.Cells.Add(cel)
                cel.Text = "Demanda Mês Atual"
                cel.Font.Bold = False
                cel.HorizontalAlign = HorizontalAlign.Center
                cel.BackColor = Drawing.Color.Gainsboro

                'COLUNA YTD
                cel = New TableCell
                rw.Cells.Add(cel)
                cel.Text = "Demanda YTD Média"
                cel.Font.Bold = False
                cel.HorizontalAlign = HorizontalAlign.Center
                cel.BackColor = Drawing.Color.Gainsboro

                'COLUNA ESTOQUE ATUAL
                cel = New TableCell
                rw.Cells.Add(cel)
                cel.Text = "Estoque Atual (Calculado)"
                cel.Font.Bold = False
                cel.HorizontalAlign = HorizontalAlign.Center
                cel.BackColor = Drawing.Color.Gainsboro

                'COLUNA Estoque para 60 dias
                cel = New TableCell
                rw.Cells.Add(cel)
                cel.Text = "Estoque Sugerido (60 dias)"
                cel.Font.Bold = False
                cel.HorizontalAlign = HorizontalAlign.Center
                cel.BackColor = Drawing.Color.Gainsboro

                'COLUNA ESTOQUE ATUAL
                cel = New TableCell
                rw.Cells.Add(cel)
                cel.Text = "Sugestão de Pedido"
                cel.Font.Bold = False
                cel.HorizontalAlign = HorizontalAlign.Center
                cel.BackColor = Drawing.Color.Gainsboro

                PRODUTO = ""
                Do While dtr_Dst.Read
                    PRODUTO = ""
                    PRODUTO = dtr_Dst("PRODUTO")
                    COD_PRODUTO = ""
                    COD_PRODUTO = dtr_Dst("COD_PRODUTO")

                    'LINHA CABECALHO
                    rw = New TableRow
                    tbl_Report.Rows.Add(rw)

                    'COLUNA NOME DO PRODUTO
                    cel = New TableCell
                    rw.Cells.Add(cel)
                    cel.Text = PRODUTO
                    cel.Font.Bold = False
                    cel.HorizontalAlign = HorizontalAlign.Left
                    cel.BackColor = Drawing.Color.White

                    'recupera venda mês atual
                    'RECUPERA venda
                    sql = ""
                    sql = "SELECT * FROM VIEW_DISTRIBUIDORES_VENDA WHERE COD_PRODUTO='" & COD_PRODUTO & "' AND GRUPO = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = '01 VENDA' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "' AND MES = " & Month(Now())

                    cmd.Connection = cnn
                    cmd.CommandText = sql
                    dtr = cmd.ExecuteReader
                    dtr.Read()
                    If dtr.HasRows Then
                        MES_ATUAL_VENDA = FormatNumber(dtr("QTD"), 0)
                    Else
                        MES_ATUAL_VENDA = 0
                    End If
                    dtr.Close()

                    'RECUPERA VENDAS
                    sql = ""
                    sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_PRODUTO_TOTAL WHERE PRODUTO='" & PRODUTO & "' AND GRUPO = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = '01 VENDA' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "'"

                    cmd.Connection = cnn
                    cmd.CommandText = sql
                    dtr = cmd.ExecuteReader
                    dtr.Read()
                    If dtr.HasRows Then
                        'CELULA JAN
                        JAN_VENDA = FormatNumber(dtr("JAN"), 0)
                        'CELULA FEV
                        FEV_VENDA = FormatNumber(dtr("FEV"), 0)
                        'CELULA MAR
                        MAR_VENDA = FormatNumber(dtr("MAR"), 0)
                        'CELULA ABR
                        ABR_VENDA = FormatNumber(dtr("ABR"), 0)
                        'CELULA MAI
                        MAI_VENDA = FormatNumber(dtr("MAI"), 0)
                        'CELULA JUN
                        JUN_VENDA = FormatNumber(dtr("JUN"), 0)
                        'CELULA JUL
                        JUL_VENDA = FormatNumber(dtr("JUL"), 0)
                        'CELULA AGO
                        AGO_VENDA = FormatNumber(dtr("AGO"), 0)
                        'CELULA SET
                        SET_VENDA = FormatNumber(dtr("SET"), 0)
                        'CELULA OUT
                        OUT_VENDA = FormatNumber(dtr("OUT"), 0)
                        'CELULA NOV
                        NOV_VENDA = FormatNumber(dtr("NOV"), 0)
                        'CELULA DEZ
                        DEZ_VENDA = FormatNumber(dtr("DEZ"), 0)
                        'CELULA YTD
                        cel = New TableCell
                        rw.Cells.Add(cel)
                        cel.Text = FormatNumber(dtr("YTD"), 0)
                        cel.HorizontalAlign = HorizontalAlign.Center
                        YTD_VENDA = FormatNumber(dtr("YTD"), 0)

                        'CELULA VENDA MES ATUAL
                        cel = New TableCell
                        rw.Cells.Add(cel)
                        cel.Text = MES_ATUAL_VENDA
                        cel.HorizontalAlign = HorizontalAlign.Center

                        'CELULA MEDIA
                        cel = New TableCell
                        rw.Cells.Add(cel)
                        cel.Text = FormatNumber(dtr("MEDIA_YTD"), 0)
                        cel.HorizontalAlign = HorizontalAlign.Center
                        YTD_VENDA_MEDIA = FormatNumber(dtr("MEDIA_YTD"), 0)
                        'CELULA TOTAL
                        TOTAL_VENDA = JAN_VENDA + FEV_VENDA + MAR_VENDA + ABR_VENDA + MAI_VENDA + JUN_VENDA + JUL_VENDA + AGO_VENDA + SET_VENDA + OUT_VENDA + NOV_VENDA + DEZ_VENDA
                    Else
                        For X = 1 To 2
                            cel = New TableCell
                            rw.Cells.Add(cel)
                            cel.Text = 0
                            cel.HorizontalAlign = HorizontalAlign.Center
                        Next

                        JAN_VENDA = 0
                        FEV_VENDA = 0
                        MAR_VENDA = 0
                        ABR_VENDA = 0
                        MAI_VENDA = 0
                        JUN_VENDA = 0
                        JUL_VENDA = 0
                        AGO_VENDA = 0
                        SET_VENDA = 0
                        OUT_VENDA = 0
                        NOV_VENDA = 0
                        DEZ_VENDA = 0
                        TOTAL_VENDA = 0
                        YTD_VENDA = 0
                        YTD_VENDA_MEDIA = 0
                    End If
                    dtr.Close()

                    'recupera demanda mês atual
                    'RECUPERA DEMANDA
                    sql = ""
                    sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_DETALHADO WHERE COD_PRODUTO='" & COD_PRODUTO & "' AND GRUPO = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = '02 DEMANDA' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "' AND MES = " & Month(Now())

                    cmd.Connection = cnn
                    cmd.CommandText = sql
                    dtr = cmd.ExecuteReader
                    dtr.Read()
                    If dtr.HasRows Then
                        MES_ATUAL_DEMANDA = FormatNumber(dtr("QTD"), 0)
                    Else
                        MES_ATUAL_DEMANDA = 0
                    End If
                    dtr.Close()

                    'RECUPERA DEMANDA
                    sql = ""
                    sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_PRODUTO_TOTAL WHERE PRODUTO='" & PRODUTO & "' AND GRUPO = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = '02 DEMANDA' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "'"

                    cmd.Connection = cnn
                    cmd.CommandText = sql
                    dtr = cmd.ExecuteReader
                    dtr.Read()
                    If dtr.HasRows Then
                        'CELULA JAN
                        JAN_DEMANDA = FormatNumber(dtr("JAN"), 0)
                        'CELULA FEV
                        FEV_DEMANDA = FormatNumber(dtr("FEV"), 0)
                        'CELULA MAR
                        MAR_DEMANDA = FormatNumber(dtr("MAR"), 0)
                        'CELULA ABR
                        ABR_DEMANDA = FormatNumber(dtr("ABR"), 0)
                        'CELULA MAI
                        MAI_DEMANDA = FormatNumber(dtr("MAI"), 0)
                        'CELULA JUN
                        JUN_DEMANDA = FormatNumber(dtr("JUN"), 0)
                        'CELULA JUL
                        JUL_DEMANDA = FormatNumber(dtr("JUL"), 0)
                        'CELULA AGO
                        AGO_DEMANDA = FormatNumber(dtr("AGO"), 0)
                        'CELULA SET
                        SET_DEMANDA = FormatNumber(dtr("SET"), 0)
                        'CELULA OUT
                        OUT_DEMANDA = FormatNumber(dtr("OUT"), 0)
                        'CELULA NOV
                        NOV_DEMANDA = FormatNumber(dtr("NOV"), 0)
                        'CELULA DEZ
                        DEZ_DEMANDA = FormatNumber(dtr("DEZ"), 0)


                        'CELULA YTD
                        cel = New TableCell
                        rw.Cells.Add(cel)
                        cel.Text = FormatNumber(dtr("YTD"), 0)
                        cel.HorizontalAlign = HorizontalAlign.Center
                        YTD_DEMANDA = FormatNumber(dtr("YTD"), 0)

                        'CELULA DEMANDA MES ATUAL
                        cel = New TableCell
                        rw.Cells.Add(cel)
                        cel.Text = MES_ATUAL_DEMANDA
                        cel.HorizontalAlign = HorizontalAlign.Center

                        'CELULA MEDIA
                        cel = New TableCell
                        rw.Cells.Add(cel)
                        cel.Text = FormatNumber(dtr("MEDIA_YTD"), 0)
                        cel.HorizontalAlign = HorizontalAlign.Center
                        YTD_DEMANDA_MEDIA = FormatNumber(dtr("MEDIA_YTD"), 0)

                        'CELULA TOTAL
                        TOTAL_DEMANDA = JAN_DEMANDA + FEV_DEMANDA + MAR_DEMANDA + ABR_DEMANDA + MAI_DEMANDA + JUN_DEMANDA + JUL_DEMANDA + AGO_DEMANDA + SET_DEMANDA + OUT_DEMANDA + NOV_DEMANDA + DEZ_DEMANDA
                    Else
                        For X = 1 To 2
                            cel = New TableCell
                            rw.Cells.Add(cel)
                            cel.Text = 0
                            cel.HorizontalAlign = HorizontalAlign.Center
                        Next
                        JAN_DEMANDA = 0
                        FEV_DEMANDA = 0
                        MAR_DEMANDA = 0
                        ABR_DEMANDA = 0
                        MAI_DEMANDA = 0
                        JUN_DEMANDA = 0
                        JUL_DEMANDA = 0
                        AGO_DEMANDA = 0
                        SET_DEMANDA = 0
                        OUT_DEMANDA = 0
                        NOV_DEMANDA = 0
                        DEZ_DEMANDA = 0
                        TOTAL_DEMANDA = 0
                        YTD_DEMANDA = 0
                        YTD_DEMANDA_MEDIA = 0
                    End If
                    dtr.Close()
                    
                    'RECUPERA ESTOQUE INICIAL DO ANO
                    sql = ""
                    sql = "SELECT * FROM TBL_MOVIMENTO_ESTOQUE_INICIAL WHERE GRUPO_DISTRIBUIDOR = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = '03 ESTOQUE' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "'"

                    cmd.Connection = cnn
                    cmd.CommandText = sql
                    dtr = cmd.ExecuteReader
                    dtr.Read()
                    If dtr.HasRows Then
                        ESTOQUE_INICIAL = 0
                        ESTOQUE_INICIAL = FormatNumber(dtr("QTD"), 0)
                    End If
                    dtr.Close()

                    'RECUPERA ESTOQUE 
                    'SELECIONA O ANO MANUALMENTE PARA PODER TRAZER DISTRIBUIDORES SEM ESTOQUE INFORMADO NO ANO ATUAL
                    If ANO = 2015 Then
                        sql = ""
                        sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_PRODUTO_TOTAL WHERE PRODUTO='" & PRODUTO & "' AND GRUPO = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = '03 ESTOQUE' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "'"
                    Else
                        sql = ""
                        sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_PRODUTO_TOTAL WHERE PRODUTO='" & PRODUTO & "' AND GRUPO = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = '03 ESTOQUE' AND ANO = " & ANO & " OR ANO = 2015 AND LINHA = '" & LINHA_PRODUTO & "'"
                    End If

                    cmd.Connection = cnn
                    cmd.CommandText = sql
                    dtr = cmd.ExecuteReader
                    dtr.Read()
                    If dtr.HasRows Then
                        If ANO = 2015 Then
                            'CELULA JAN
                            JAN_ESTOQUE = FormatNumber(dtr("JAN"), 0)
                            'CELULA FEV
                            FEV_ESTOQUE = FormatNumber(dtr("FEV"), 0)
                            'CELULA MAR
                            MAR_ESTOQUE = FEV_ESTOQUE + MAR_VENDA - MAR_DEMANDA
                            'CELULA ABR
                            ABR_ESTOQUE = MAR_ESTOQUE + ABR_VENDA - ABR_DEMANDA
                            'CELULA MAI
                            MAI_ESTOQUE = ABR_ESTOQUE + MAI_VENDA - MAI_DEMANDA
                            'CELULA JUN
                            JUN_ESTOQUE = MAI_ESTOQUE + JUN_VENDA - JUN_DEMANDA
                            'CELULA JUL
                            JUL_ESTOQUE = JUN_ESTOQUE + JUL_VENDA - JUL_DEMANDA
                            'CELULA AGO
                            AGO_ESTOQUE = JUL_ESTOQUE + AGO_VENDA - AGO_DEMANDA
                            'CELULA SET
                            SET_ESTOQUE = AGO_ESTOQUE + SET_VENDA - SET_DEMANDA
                            'CELULA OUT
                            OUT_ESTOQUE = SET_ESTOQUE + OUT_VENDA - OUT_DEMANDA
                            'CELULA NOV
                            NOV_ESTOQUE = OUT_ESTOQUE + NOV_VENDA - NOV_DEMANDA
                            'CELULA DEZ (Estoque Atual)

                            cel = New TableCell
                            rw.Cells.Add(cel)
                            cel.HorizontalAlign = HorizontalAlign.Center
                            cel.BackColor = Drawing.Color.White
                            cel.ForeColor = Drawing.Color.DimGray
                            DEZ_ESTOQUE = NOV_ESTOQUE + DEZ_VENDA - DEZ_DEMANDA
                            cel.Text = FormatNumber(DEZ_ESTOQUE, 0)

                        ElseIf ANO = Year(Now()) Then
                            'CELULA JAN
                            JAN_ESTOQUE = ESTOQUE_INICIAL + JAN_VENDA - JAN_DEMANDA
                            'CELULA FEV
                            FEV_ESTOQUE = JAN_ESTOQUE + FEV_VENDA - FEV_DEMANDA
                            'CELULA MAR
                            MAR_ESTOQUE = FEV_ESTOQUE + MAR_VENDA - MAR_DEMANDA
                            'CELULA ABR
                            ABR_ESTOQUE = MAR_ESTOQUE + ABR_VENDA - ABR_DEMANDA
                            'CELULA MAI
                            MAI_ESTOQUE = ABR_ESTOQUE + MAI_VENDA - MAI_DEMANDA
                            'CELULA JUN
                            JUN_ESTOQUE = MAI_ESTOQUE + JUN_VENDA - JUN_DEMANDA
                            'CELULA JUL
                            JUL_ESTOQUE = JUN_ESTOQUE + JUL_VENDA - JUL_DEMANDA
                            'CELULA AGO
                            AGO_ESTOQUE = JUL_ESTOQUE + AGO_VENDA - AGO_DEMANDA
                            'CELULA SET
                            SET_ESTOQUE = AGO_ESTOQUE + SET_VENDA - SET_DEMANDA
                            'CELULA OUT
                            OUT_ESTOQUE = SET_ESTOQUE + OUT_VENDA - OUT_DEMANDA
                            'CELULA NOV
                            NOV_ESTOQUE = OUT_ESTOQUE + NOV_VENDA - NOV_DEMANDA
                            'CELULA DEZ (Estoque Atual)
                            cel = New TableCell
                            rw.Cells.Add(cel)
                            cel.HorizontalAlign = HorizontalAlign.Center
                            cel.BackColor = Drawing.Color.White
                            cel.ForeColor = Drawing.Color.DimGray
                            cel.Text = FormatNumber(DEZ_ESTOQUE, 0)
                            DEZ_ESTOQUE = NOV_ESTOQUE + DEZ_VENDA - DEZ_DEMANDA

                        End If
                    Else
                        JAN_ESTOQUE = 0
                        FEV_ESTOQUE = 0
                        MAR_ESTOQUE = 0
                        ABR_ESTOQUE = 0
                        MAI_ESTOQUE = 0
                        JUN_ESTOQUE = 0
                        JUL_ESTOQUE = 0
                        AGO_ESTOQUE = 0
                        SET_ESTOQUE = 0
                        OUT_ESTOQUE = 0
                        NOV_ESTOQUE = 0
                        DEZ_ESTOQUE = 0
                        TOTAL_ESTOQUE = 0
                    End If
                    dtr.Close()

                    cel = New TableCell
                    rw.Cells.Add(cel)
                    If 60 * (YTD_DEMANDA_MEDIA / 30) < 0 Then
                        cel.Text = FormatNumber((60 * (YTD_DEMANDA_MEDIA / 30)), 0)
                    ElseIf 60 * (YTD_DEMANDA_MEDIA / 30) > 0 Then
                        cel.Text = FormatNumber((60 * (YTD_DEMANDA_MEDIA / 30)), 0)
                    Else
                        cel.Text = 0
                    End If
                    cel.Text = FormatNumber((60 * (YTD_DEMANDA_MEDIA / 30)), 0)
                    cel.HorizontalAlign = HorizontalAlign.Center
                    cel.BackColor = Drawing.Color.White
                    cel.ForeColor = Drawing.Color.DimGray

                    cel = New TableCell
                    rw.Cells.Add(cel)
                    If 60 * (YTD_DEMANDA_MEDIA / 30) < 0 Then
                        cel.Text = FormatNumber((60 * (YTD_DEMANDA_MEDIA / 30)) - DEZ_ESTOQUE, 0)
                    ElseIf 60 * (YTD_DEMANDA_MEDIA / 30) > 0 Then
                        cel.Text = FormatNumber((60 * (YTD_DEMANDA_MEDIA / 30)) - DEZ_ESTOQUE, 0)
                    Else
                        cel.Text = 0
                    End If
                    cel.HorizontalAlign = HorizontalAlign.Center
                    cel.BackColor = Drawing.Color.White
                    cel.ForeColor = Drawing.Color.DimGray

                Loop
                dtr_Dst.Close()
            End If
        Loop

        dtr_Linha.Close()
    End Sub
    Protected Function Alert(ByVal Texto_Mensagem As String, ByVal Redirecionar As Boolean, ByVal Pagina As String) As Boolean
        Dim jscript As String
        'caixa de mensagem
        If Redirecionar = True Then
            jscript = ""
            jscript += "<script language='JavaScript'>"
            jscript += ";alert('" & Texto_Mensagem & "'); document.location.href='" & Pagina & "'"
            jscript += "</script>"
        Else
            jscript = ""
            jscript += "<script language='JavaScript'>"
            jscript += ";alert('" & Texto_Mensagem & "');"
            jscript += "</script>"
        End If
        Page.ClientScript.RegisterClientScriptBlock(Me.GetType(), "Mensagem", jscript)
        Alert = True
    End Function
    Protected Sub cmb_DISTRIBUIDOR_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_DISTRIBUIDOR.SelectedIndexChanged
        Atualiza_Relatorio()
    End Sub
End Class