...
Partial Class Produtos_Cadastro_Divisao
    Inherits System.Web.UI.Page
    'Váriaveis Padrão
    Dim M As New clsMaster
    Dim Pagina As String = "Produtos_Cadastro_Divisao.aspx"
    Dim Titulo As String = "Divisões de Produto - " & ConfigurationManager.AppSettings("NOME_SISTEMA").ToString
    ...
    Protected Sub cmd_Gravar_Click(sender As Object, e As EventArgs) Handles cmd_Gravar.Click
        If Len(txt_Divisao.Text) = 0 Then
            Alert("Digite a divisão de produto antes de gravar!", False, "")
            Exit Sub
        End If
        Sql = ""
        Sql = "Insert Into TBL_PRODUTOS_DIVISAO (PRODUTO_DIVISAO, INCLUSAO_EMAIL, INCLUSAO_DATA) Values "
        Sql = Sql & "('" & M.ConverteTexto(txt_Divisao.Text) & "', "
        ...
        If M.ExecutarSQL(Sql) = True Then
            ...
            dts_Lista.DataBind()
            gdv_Lista.DataBind()
            M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "Inclui divisão de produto", txt_Divisao.Text)
            Alert("Divisão de Produto incluida com sucesso!", False, "")
        Else
            Mensagem = "ERRO - INCLUSAO DIVISÃO DE PRODUTO - " & Pagina & " - " & Now() & Chr(10)
            Mensagem = Mensagem & "DIVISÃO DE PRODUTO: " & txt_Divisao.Text & Chr(10)
            Mensagem = Mensagem & "Por: " & Session("USUARIO_LOGIN") & " - " & Session("EMAIL_LOGIN")
            M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "ERRO", txt_Divisao.Text)
            M.EnviarEmail("ERRO - INCLUSAO DIVISÃO DE PRODUTO", Mensagem, True, False, "", "")
    ...
    Protected Sub gdv_Lista_RowUpdated(sender As Object, e As GridViewUpdatedEventArgs) Handles gdv_Lista.RowUpdated
        M.Inclui_Alteracao(gdv_Lista.SelectedValue, Session("EMAIL_LOGIN"), "COD_PRODUTO_DIVISAO", "TBL_PRODUTOS_DIVISAO")
    End Sub
    Protected Sub gdv_Lista_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gdv_Lista.SelectedIndexChanged
        ...
        QTD_REGISTROS = M.Consiste_Exclusao(gdv_Lista.SelectedValue, "COD_PRODUTO_DIVISAO", "TBL_PRODUTOS")
        If QTD_REGISTROS > 0 Then
            Alert("Esta divisão não pode ser excluido pois existem " & QTD_REGISTROS & " produtos relacionados", False, "")
        Else
            If M.ExecutarSQL("Delete From TBL_PRODUTOS_DIVISAO Where COD_PRODUTO_DIVISAO = " & gdv_Lista.SelectedValue) = True Then
                M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "Excluiu Divisão de produto COD", gdv_Lista.SelectedValue)
                Alert("Divisão excluida com sucesso ", False, "")
                dts_Lista.DataBind()
                gdv_Lista.DataBind()
            End If
        End If
