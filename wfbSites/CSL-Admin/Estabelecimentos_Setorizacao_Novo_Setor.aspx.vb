...
Partial Class Estabelecimentos_Setorizacao_Novo_Setor
    Inherits System.Web.UI.Page
    'Váriaveis Padrão
    Dim M As New clsMaster
    Public jscript As String
    Dim Pagina As String = "Estabelecimentos_Setorizacao_Novo_Setor.aspx"
    Dim Titulo As String = "Novo Setor - " & ConfigurationManager.AppSettings("NOME_SISTEMA").ToString
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("COD_PERFIL_LOGIN") = "0" Then sql = "SELECT '@' AS EMAIL, '# Selecione' as NOME UNION ALL Select EMAIL, NOME From VIEW_USUARIOS Where COD_PERFIL = '4' ORDER BY NOME"
        If Session("COD_PERFIL_LOGIN") = "1" Then sql = "SELECT '@' AS EMAIL, '# Selecione' as NOME UNION ALL Select EMAIL, NOME From VIEW_USUARIOS Where EMAIL_DIRETOR= '" & Session("EMAIL_LOGIN") & "' AND COD_PERFIL = '4' ORDER BY NOME"
        If Session("COD_PERFIL_LOGIN") = "2" Then sql = "SELECT '@' AS EMAIL, '# Selecione' as NOME UNION ALL Select EMAIL, NOME From VIEW_USUARIOS Where EMAIL_GERENTE= '" & Session("EMAIL_LOGIN") & "' AND COD_PERFIL = '4' ORDER BY NOME"
        If Session("COD_PERFIL_LOGIN") = "3" Then sql = "SELECT '@' AS EMAIL, '# Selecione' as NOME UNION ALL Select EMAIL, NOME From VIEW_USUARIOS Where EMAIL_COORDENADOR= '" & Session("EMAIL_LOGIN") & "' AND COD_PERFIL = '4'  ORDER BY NOME"
        If Session("COD_PERFIL_LOGIN") = "4" Then sql = "SELECT '@' AS EMAIL, '# Selecione' as NOME UNION ALL Select EMAIL, NOME From VIEW_USUARIOS Where EMAIL = '" & Session("EMAIL_LOGIN") & "' ORDER BY NOME"
    ...
    Protected Sub cmd_Gravar_Click(sender As Object, e As System.EventArgs) Handles cmd_Gravar.Click
        ...
        For Each gdvrow As GridViewRow In gdv_Localizar.Rows
            ...
            SETORIZAR = CType(gdvrow.Cells(5).FindControl("chk_Setorizar"), CheckBox).Checked()
            CNPJ = gdvrow.Cells(0).Text

            If cmb_EMAIL_REPRESENTANTE.Text = "@" Then
                Alert("Selecione o Representante para que deseja criar o Setor!", False, "")
                Atualiza_Relatório()
            Else
                If SETORIZAR = True Then

                    sql = "SELECT CNPJ FROM TBL_SETORIZACAO WHERE CNPJ = " & M.Converte_Valor(CNPJ) & " AND EMAIL = '" & cmb_EMAIL_REPRESENTANTE.Text & "'"
                    cmd.Connection = cnn
                    cmd.CommandText = sql
                    ...
                    Else
                        ...
                        sql = sql & "'" & cmb_EMAIL_REPRESENTANTE.Text & "', "
                        sql = sql & "'" & TARGET & "', "
