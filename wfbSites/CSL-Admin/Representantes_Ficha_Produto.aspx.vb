Imports System.Math
Partial Class Representates_Ficha_Produto
    Inherits System.Web.UI.Page
    Dim M As New clsMaster
    Dim Pagina As String = "Representantes_Ficha_Produto.aspx"
    ...
    Dim cnn_produto As New System.Data.SqlClient.SqlConnection
    Dim cmd_produto As New System.Data.SqlClient.SqlCommand
    Dim dtr_produto As System.Data.SqlClient.SqlDataReader
    ...
    Dim VIEW_TIPO_MOVIMENTO As String
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("COD_PERFIL_LOGIN") = "0" Then sql = "Select * From VIEW_USUARIOS Where COD_PERFIL = '4' ORDER BY NOME"
        If Session("COD_PERFIL_LOGIN") = "1" Then sql = "Select * From VIEW_USUARIOS Where EMAIL_DIRETOR= '" & Session("EMAIL_LOGIN") & "' and COD_PERFIL = '4' ORDER BY NOME"
        If Session("COD_PERFIL_LOGIN") = "2" Then sql = "Select * From VIEW_USUARIOS Where EMAIL_GERENTE= '" & Session("EMAIL_LOGIN") & "' ORDER BY NOME"
        If Session("COD_PERFIL_LOGIN") = "3" Then Alert("Você não tem acesso a esta página", True, "Default.aspx")
        If Session("COD_PERFIL_LOGIN") = "4" Then sql = "SELECT '@' AS EMAIL, '@Selecione' as NOME UNION ALL Select DISTINCT EMAIL as EMAIL, NOME as NOME From VIEW_USUARIOS Where EMAIL = '" & Session("EMAIL_LOGIN") & "'  ORDER BY NOME"
    ...
    Protected Sub cmb_Produto_Linha_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_Produto_Linha.SelectedIndexChanged
        Response.Redirect("Representantes_Ficha_Produto.aspx?REPRESENTANTE=" & cmb_EMAIL_REPRESENTANTE.Text & "&TIPO_MOVIMENTO=" & cmb_TIPO_MOVIMENTO.Text & "&LINHA=" & cmb_Produto_Linha.Text & "&ANO=" & cmb_Ano.Text)
    End Sub
    ...
    Protected Sub cmb_TIPO_MOVIMENTO_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_TIPO_MOVIMENTO.SelectedIndexChanged
        Response.Redirect("Representantes_Ficha_Produto.aspx?REPRESENTANTE=" & cmb_EMAIL_REPRESENTANTE.Text & "&TIPO_MOVIMENTO=" & cmb_TIPO_MOVIMENTO.Text & "&LINHA=" & Request.QueryString("LINHA") & "&ANO=" & cmb_Ano.Text)
    End Sub
    Public Sub Atualiza_Relatorio()
        ...
        If Request.QueryString("TIPO_MOVIMENTO") <> "" Then
            cmb_TIPO_MOVIMENTO.Text = Request.QueryString("TIPO_MOVIMENTO")
            cmb_TIPO_MOVIMENTO.DataBind()
            VIEW_TIPO_MOVIMENTO = cmb_TIPO_MOVIMENTO.Text
            If Request.QueryString("LINHA") = "@" Then
                Alert("Selecione um tipo de movimento!", False, "")
                Exit Sub
            End If
        End If
        If VIEW_TIPO_MOVIMENTO <> "@" And VIEW_TIPO_MOVIMENTO <> "" Then
            'ABRE CONEXÃO
            ...
            cnn_produto.ConnectionString = ConfigurationManager.ConnectionStrings("cnnStr").ConnectionString
            cnn_produto.Open()
            'DEFINE VARIAVEIS
            ANO_ATUAL = 0
            ...
            If Request.QueryString("LINHA") = "# Todas as Linhas" Then
                sql_linha_produto = "Select * From TBL_PRODUTOS_LINHAS Where Analisar = 'True'"
            Else
            ...
            If dtr_linha_produto.HasRows Then
                ...
                Do While dtr_linha_produto.Read
                    ...
                    'MONTA PERFORMANCE DOS GERENTES REGIONAIS 
                    sql_produto = "Select Distinct PRODUTO FROM " & VIEW_TIPO_MOVIMENTO & " WHERE ANO = " & ANO_ATUAL & " AND EMAIL_REPRESENTANTE <> 'distribuidores@wfbconsultoria.com.br' AND PRODUTO_LINHA = '" & dtr_linha_produto("PRODUTO_LINHA") & "' ORDER BY PRODUTO"
                    cmd_produto.Connection = cnn_produto
                    cmd_produto.CommandText = sql_produto
                    dtr_produto = cmd_produto.ExecuteReader
                    Linha_Vazia(30, "", True)

                    Linha_Titulo(dtr_linha_produto("PRODUTO_LINHA") & " - " & "Produtos")
                    While dtr_produto.Read()

                        If Not IsDBNull(dtr_produto("PRODUTO")) Then
                            PRODUTO = dtr_produto("PRODUTO")
                        Else
                            PRODUTO = ""
                        End If

                        Linha_Vazia(30, "", True)
                        Linha_Titulo(PRODUTO)
                        Linha_Cabecalho_Meses("Demanda Histórico")
                        Linha_Demanda_Atual_Produto(dtr_linha_produto("PRODUTO_LINHA"), ANO_ATUAL, True)
                        Linha_Demanda_Passado_Produto(dtr_linha_produto("PRODUTO_LINHA"), ANO_PASSADO, True)
                        Linha_Demanda_Retrasado_PRODUTO(dtr_linha_produto("PRODUTO_LINHA"), ANO_RETRASADO, True)
    ...
    Private Sub Linha_Demanda_Retrasado(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "FROM dbo." & VIEW_TIPO_MOVIMENTO & " "
        sql = sql & "WHERE (ANO = " & ANO_RETRASADO & ") AND (PRODUTO_LINHA = '" & LINHA_PRODUTO & "') AND EMAIL_REPRESENTANTE <> 'distribuidores@wfbconsultoria.com.br'"
        ...
        End If
        dtr.Close()
        TRI01_DEMANDA_RETRASADO = JUL_DEMANDA_RETRASADO + AGO_DEMANDA_RETRASADO + SET_DEMANDA_RETRASADO
        TRI02_DEMANDA_RETRASADO = OUT_DEMANDA_RETRASADO + NOV_DEMANDA_RETRASADO + DEZ_DEMANDA_RETRASADO
        TRI03_DEMANDA_RETRASADO = JAN_DEMANDA_RETRASADO + FEV_DEMANDA_RETRASADO + MAR_DEMANDA_RETRASADO
        TRI04_DEMANDA_RETRASADO = ABR_DEMANDA_RETRASADO + MAI_DEMANDA_RETRASADO + JUN_DEMANDA_RETRASADO
        ...
        If MES_ATUAL = 2 Then YTD_DEMANDA_RETRASADO = JUL_DEMANDA_RETRASADO
        If MES_ATUAL = 3 Then YTD_DEMANDA_RETRASADO = JUL_DEMANDA_RETRASADO + AGO_DEMANDA_RETRASADO
        If MES_ATUAL = 4 Then YTD_DEMANDA_RETRASADO = TRI01_DEMANDA_RETRASADO
        If MES_ATUAL = 5 Then YTD_DEMANDA_RETRASADO = TRI01_DEMANDA_RETRASADO + OUT_DEMANDA_RETRASADO
        If MES_ATUAL = 6 Then YTD_DEMANDA_RETRASADO = TRI01_DEMANDA_RETRASADO + OUT_DEMANDA_RETRASADO + NOV_DEMANDA_RETRASADO
        If MES_ATUAL = 7 Then YTD_DEMANDA_RETRASADO = TRI01_DEMANDA_RETRASADO + TRI02_DEMANDA_RETRASADO
        If MES_ATUAL = 8 Then YTD_DEMANDA_RETRASADO = TRI01_DEMANDA_RETRASADO + TRI02_DEMANDA_RETRASADO + JAN_DEMANDA_RETRASADO
        If MES_ATUAL = 9 Then YTD_DEMANDA_RETRASADO = TRI01_DEMANDA_RETRASADO + TRI02_DEMANDA_RETRASADO + JAN_DEMANDA_RETRASADO + FEV_DEMANDA_RETRASADO
        If MES_ATUAL = 10 Then YTD_DEMANDA_RETRASADO = TRI01_DEMANDA_RETRASADO + TRI02_DEMANDA_RETRASADO + TRI03_DEMANDA_RETRASADO
        If MES_ATUAL = 11 Then YTD_DEMANDA_RETRASADO = TRI01_DEMANDA_RETRASADO + TRI02_DEMANDA_RETRASADO + TRI03_DEMANDA_RETRASADO + ABR_DEMANDA_RETRASADO
        If MES_ATUAL = 12 Then YTD_DEMANDA_RETRASADO = TRI01_DEMANDA_RETRASADO + TRI02_DEMANDA_RETRASADO + TRI03_DEMANDA_RETRASADO + ABR_DEMANDA_RETRASADO + MAI_DEMANDA_RETRASADO
    ...
    End Sub
    Private Sub Linha_Demanda_Retrasado_PRODUTO(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        'RECUPERA INFORMAÇÕES
        JAN_DEMANDA_RETRASADO = 0
        FEV_DEMANDA_RETRASADO = 0
        MAR_DEMANDA_RETRASADO = 0
        ...
        sql = sql & "SUM(AGO) AS AGO, SUM([SET]) AS [SET], SUM(OUT) AS OUT, SUM(NOV) AS NOV, SUM(DEZ) AS DEZ "
        sql = sql & "FROM dbo." & VIEW_TIPO_MOVIMENTO & " "
        sql = sql & "WHERE (ANO = " & ANO_RETRASADO & ") AND (PRODUTO_LINHA = '" & LINHA_PRODUTO & "') "
        sql = sql & "AND EMAIL_REPRESENTANTE <> 'distribuidores@wfbconsultoria.com.br' AND PRODUTO = '" & PRODUTO & "'"
    ...
    Private Sub Linha_Demanda_Passado(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "SUM(AGO) AS AGO, SUM([SET]) AS [SET], SUM(OUT) AS OUT, SUM(NOV) AS NOV, SUM(DEZ) AS DEZ "
        sql = sql & "FROM dbo." & VIEW_TIPO_MOVIMENTO & " "
        sql = sql & "WHERE (ANO = " & ANO_PASSADO & ") AND (PRODUTO_LINHA = '" & LINHA_PRODUTO & "') AND EMAIL_REPRESENTANTE <> 'distribuidores@wfbconsultoria.com.br'"
        ...
        End If
        dtr.Close()
        TRI01_DEMANDA_PASSADO = JUL_DEMANDA_PASSADO + AGO_DEMANDA_PASSADO + SET_DEMANDA_PASSADO
        TRI02_DEMANDA_PASSADO = OUT_DEMANDA_PASSADO + NOV_DEMANDA_PASSADO + DEZ_DEMANDA_PASSADO
        TRI03_DEMANDA_PASSADO = JAN_DEMANDA_PASSADO + FEV_DEMANDA_PASSADO + MAR_DEMANDA_PASSADO
        TRI04_DEMANDA_PASSADO = ABR_DEMANDA_PASSADO + MAI_DEMANDA_PASSADO + JUN_DEMANDA_PASSADO
        ...
        If MES_ATUAL = 2 Then YTD_DEMANDA_PASSADO = JUL_DEMANDA_PASSADO
        If MES_ATUAL = 3 Then YTD_DEMANDA_PASSADO = JUL_DEMANDA_PASSADO + AGO_DEMANDA_PASSADO
        If MES_ATUAL = 4 Then YTD_DEMANDA_PASSADO = TRI01_DEMANDA_PASSADO
        If MES_ATUAL = 5 Then YTD_DEMANDA_PASSADO = TRI01_DEMANDA_PASSADO + OUT_DEMANDA_PASSADO
        If MES_ATUAL = 6 Then YTD_DEMANDA_PASSADO = TRI01_DEMANDA_PASSADO + OUT_DEMANDA_PASSADO + NOV_DEMANDA_PASSADO
        If MES_ATUAL = 7 Then YTD_DEMANDA_PASSADO = TRI01_DEMANDA_PASSADO + TRI02_DEMANDA_PASSADO
        If MES_ATUAL = 8 Then YTD_DEMANDA_PASSADO = TRI01_DEMANDA_PASSADO + TRI02_DEMANDA_PASSADO + JAN_DEMANDA_PASSADO
        If MES_ATUAL = 9 Then YTD_DEMANDA_PASSADO = TRI01_DEMANDA_PASSADO + TRI02_DEMANDA_PASSADO + JAN_DEMANDA_PASSADO + FEV_DEMANDA_PASSADO
        If MES_ATUAL = 10 Then YTD_DEMANDA_PASSADO = TRI01_DEMANDA_PASSADO + TRI02_DEMANDA_PASSADO + TRI03_DEMANDA_PASSADO
        If MES_ATUAL = 11 Then YTD_DEMANDA_PASSADO = TRI01_DEMANDA_PASSADO + TRI02_DEMANDA_PASSADO + TRI03_DEMANDA_PASSADO + ABR_DEMANDA_PASSADO
        If MES_ATUAL = 12 Then YTD_DEMANDA_PASSADO = TRI01_DEMANDA_PASSADO + TRI02_DEMANDA_PASSADO + TRI03_DEMANDA_PASSADO + ABR_DEMANDA_PASSADO + MAI_DEMANDA_PASSADO
    ...
    End Sub
    Private Sub Linha_Demanda_Passado_Produto(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        'RECUPERA INFORMAÇÕES
        JAN_DEMANDA_PASSADO = 0
        FEV_DEMANDA_PASSADO = 0
        MAR_DEMANDA_PASSADO = 0
        ...
        sql = sql & "SUM(AGO) AS AGO, SUM([SET]) AS [SET], SUM(OUT) AS OUT, SUM(NOV) AS NOV, SUM(DEZ) AS DEZ "
        sql = sql & "FROM dbo." & VIEW_TIPO_MOVIMENTO & " "
        sql = sql & "WHERE (ANO = " & ANO_PASSADO & ") AND (PRODUTO_LINHA = '" & LINHA_PRODUTO & "')"
        sql = sql & "AND EMAIL_REPRESENTANTE <> 'distribuidores@wfbconsultoria.com.br' AND PRODUTO = '" & PRODUTO & "'"
    ...
    Private Sub Linha_Demanda_Atual(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "SUM(AGO) AS AGO, SUM([SET]) AS [SET], SUM(OUT) AS OUT, SUM(NOV) AS NOV, SUM(DEZ) AS DEZ "
        sql = sql & "FROM dbo." & VIEW_TIPO_MOVIMENTO & " "
        sql = sql & "WHERE (ANO = " & ANO_ATUAL & ") AND (PRODUTO_LINHA = '" & LINHA_PRODUTO & "') AND EMAIL_REPRESENTANTE <> 'distribuidores@wfbconsultoria.com.br'"
        cmd.Connection = cnn
        cmd.CommandText = sql
        dtr = cmd.ExecuteReader
        dtr.Read()
        ...
        End If
        dtr.Close()
        TRI01_DEMANDA_ATUAL = JUL_DEMANDA_ATUAL + AGO_DEMANDA_ATUAL + SET_DEMANDA_ATUAL
        TRI02_DEMANDA_ATUAL = OUT_DEMANDA_ATUAL + NOV_DEMANDA_ATUAL + DEZ_DEMANDA_ATUAL
        TRI03_DEMANDA_ATUAL = JAN_DEMANDA_ATUAL + FEV_DEMANDA_ATUAL + MAR_DEMANDA_ATUAL
        TRI04_DEMANDA_ATUAL = ABR_DEMANDA_ATUAL + MAI_DEMANDA_ATUAL + JUN_DEMANDA_ATUAL
        ...
        If MES_ATUAL = 2 Then YTD_DEMANDA_ATUAL = JUL_DEMANDA_ATUAL
        If MES_ATUAL = 3 Then YTD_DEMANDA_ATUAL = JUL_DEMANDA_ATUAL + AGO_DEMANDA_ATUAL
        If MES_ATUAL = 4 Then YTD_DEMANDA_ATUAL = TRI01_DEMANDA_ATUAL
        If MES_ATUAL = 5 Then YTD_DEMANDA_ATUAL = TRI01_DEMANDA_ATUAL + OUT_DEMANDA_ATUAL
        If MES_ATUAL = 6 Then YTD_DEMANDA_ATUAL = TRI01_DEMANDA_ATUAL + OUT_DEMANDA_ATUAL + NOV_DEMANDA_ATUAL
        If MES_ATUAL = 7 Then YTD_DEMANDA_ATUAL = TRI01_DEMANDA_ATUAL + TRI02_DEMANDA_ATUAL
        If MES_ATUAL = 8 Then YTD_DEMANDA_ATUAL = TRI01_DEMANDA_ATUAL + TRI02_DEMANDA_ATUAL + JAN_DEMANDA_ATUAL
        If MES_ATUAL = 9 Then YTD_DEMANDA_ATUAL = TRI01_DEMANDA_ATUAL + TRI02_DEMANDA_ATUAL + JAN_DEMANDA_ATUAL + FEV_DEMANDA_ATUAL
        If MES_ATUAL = 10 Then YTD_DEMANDA_ATUAL = TRI01_DEMANDA_ATUAL + TRI02_DEMANDA_ATUAL + TRI03_DEMANDA_ATUAL
        If MES_ATUAL = 11 Then YTD_DEMANDA_ATUAL = TRI01_DEMANDA_ATUAL + TRI02_DEMANDA_ATUAL + TRI03_DEMANDA_ATUAL + ABR_DEMANDA_ATUAL
        If MES_ATUAL = 12 Then YTD_DEMANDA_ATUAL = TRI01_DEMANDA_ATUAL + TRI02_DEMANDA_ATUAL + TRI03_DEMANDA_ATUAL + ABR_DEMANDA_ATUAL + MAI_DEMANDA_ATUAL
    ...
    End Sub
    Private Sub Linha_Demanda_Atual_Produto(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        'RECUPERA INFORMAÇÕES
        JAN_DEMANDA_ATUAL = 0
        FEV_DEMANDA_ATUAL = 0
        MAR_DEMANDA_ATUAL = 0
        ...
        sql = sql & "SUM(AGO) AS AGO, SUM([SET]) AS [SET], SUM(OUT) AS OUT, SUM(NOV) AS NOV, SUM(DEZ) AS DEZ "
        sql = sql & "FROM dbo." & VIEW_TIPO_MOVIMENTO & " "
        sql = sql & "WHERE (ANO = " & ANO_ATUAL & ") AND (PRODUTO_LINHA = '" & LINHA_PRODUTO & "') "
        sql = sql & "AND EMAIL_REPRESENTANTE <> 'distribuidores@wfbconsultoria.com.br' AND PRODUTO = '" & PRODUTO & "'"
        cmd.Connection = cnn
        cmd.CommandText = sql
        dtr = cmd.ExecuteReader
    ...
    Private Sub Linha_Cota_Atual(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        If VIEW_TIPO_MOVIMENTO = "VIEW_MOVIMENTO_DEMANDA_001_DETALHADO_FISCAL_UN_EQ" Then
            sql = sql & "FROM dbo.VIEW_MOVIMENTO_COTAS_DEMANDA_FISCAL_UN_EQ "
        ElseIf VIEW_TIPO_MOVIMENTO = "VIEW_MOVIMENTO_VENDAS_001_DETALHADO_FISCAL_VL_RS" Then
            sql = sql & "FROM dbo.VIEW_MOVIMENTO_COTAS_VENDA_FISCAL_VL_RS "
        End If
        ...
        End If
        dtr.Close()
        TRI01_COTA = JUL_COTA + AGO_COTA + SET_COTA
        TRI02_COTA = OUT_COTA + NOV_COTA + DEZ_COTA
        TRI03_COTA = JAN_COTA + FEV_COTA + MAR_COTA
        TRI04_COTA = ABR_COTA + MAI_COTA + JUN_COTA
        TOTAL_COTA = TRI01_COTA + TRI02_COTA + TRI03_COTA + TRI04_COTA

        If MES_ATUAL = 1 Then YTD_COTA = 0
        If MES_ATUAL = 2 Then YTD_COTA = JUL_COTA
        If MES_ATUAL = 3 Then YTD_COTA = JUL_COTA + AGO_COTA
        If MES_ATUAL = 4 Then YTD_COTA = TRI01_COTA
        If MES_ATUAL = 5 Then YTD_COTA = TRI01_COTA + OUT_COTA
        If MES_ATUAL = 6 Then YTD_COTA = TRI01_COTA + OUT_COTA + NOV_COTA
        If MES_ATUAL = 7 Then YTD_COTA = TRI01_COTA + TRI02_COTA
        If MES_ATUAL = 8 Then YTD_COTA = TRI01_COTA + TRI02_COTA + JAN_COTA
        If MES_ATUAL = 9 Then YTD_COTA = TRI01_COTA + TRI02_COTA + JAN_COTA + FEV_COTA
        If MES_ATUAL = 10 Then YTD_COTA = TRI01_COTA + TRI02_COTA + TRI03_COTA
        If MES_ATUAL = 11 Then YTD_COTA = TRI01_COTA + TRI02_COTA + TRI03_COTA + ABR_COTA
        If MES_ATUAL = 12 Then YTD_COTA = TRI01_COTA + TRI02_COTA + TRI03_COTA + JAN_COTA + FEV_COTA
