...
Partial Class rpt_Estoque_Linha
    ...
    Public Sub Atualiza_Relatorio()
        ...
        'COLUNAS MESES
        sql = "SELECT MES_FISCAL,MES_SIGLA FROM TBL_DATAS_MESES WHERE MES_FISCAL > 0 ORDER BY MES_FISCAL"
        cmd.Connection = cnn
        cmd.CommandText = sql
        ...
        sql = ""
        sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_TOTAL_GERAL_FISCAL_UN_OR WHERE TIPO = 'VENDA UNIDADES ORIGINAL FISCAL' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "'"
        ...
        'RECUPERA DEMANDA
        sql = ""
        sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_TOTAL_GERAL_FISCAL_UN_OR WHERE TIPO = 'DEMANDA UNIDADES ORIGINAL FISCAL' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "'"
        ...
        sql = sql & "FROM dbo.VIEW_COTA_DEMANDA_DETALHADO_FISCAL_UN_EQ_DIST_GERAL "
        sql = sql & "WHERE (ANO_FISCAL = '" & ANO & "' AND PRODUTO_LINHA = '" & LINHA_PRODUTO & "') "

        cmd.Connection = cnn
        cmd.CommandText = sql
        dtr = cmd.ExecuteReader
        dtr.Read()
        If dtr.HasRows And Not IsDBNull(dtr("JUL")) Then
        ...
        sql_Dst = "SELECT DISTINCT GRUPO FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_TOTAL_FISCAL_UN_OR WHERE ANO = " & ANO & " AND GRUPO = '01 - CSL BEHRING (Venda Direta)' ORDER BY GRUPO "
        ...
        Do While dtr_Dst.Read
            ...
            'COLUNAS MESES
            sql = "SELECT MES_SIGLA FROM TBL_DATAS_MESES WHERE MES_FISCAL > 0 ORDER BY MES_FISCAL"
            cmd.Connection = cnn
            cmd.CommandText = sql
            dtr = cmd.ExecuteReader
            ...
            sql = ""
            sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_TOTAL_FISCAL_UN_OR WHERE GRUPO = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = '02 DEMANDA' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "'"
            ...
            If JAN_VENDA_TOTAL <> 0 Then JAN_PARTICIPACAO_VENDA = Round((JAN_DEMANDA / JAN_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If FEV_VENDA_TOTAL <> 0 Then FEV_PARTICIPACAO_VENDA = Round((FEV_DEMANDA / FEV_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If MAR_VENDA_TOTAL <> 0 Then MAR_PARTICIPACAO_VENDA = Round((MAR_DEMANDA / MAR_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If ABR_VENDA_TOTAL <> 0 Then ABR_PARTICIPACAO_VENDA = Round((ABR_DEMANDA / ABR_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If MAI_VENDA_TOTAL <> 0 Then MAI_PARTICIPACAO_VENDA = Round((MAI_DEMANDA / MAI_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If JUN_VENDA_TOTAL <> 0 Then JUN_PARTICIPACAO_VENDA = Round((JUN_DEMANDA / JUN_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If JUL_VENDA_TOTAL <> 0 Then JUL_PARTICIPACAO_VENDA = Round((JUL_DEMANDA / JUL_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If AGO_VENDA_TOTAL <> 0 Then AGO_PARTICIPACAO_VENDA = Round((AGO_DEMANDA / AGO_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If SET_VENDA_TOTAL <> 0 Then SET_PARTICIPACAO_VENDA = Round((SET_DEMANDA / SET_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If OUT_VENDA_TOTAL <> 0 Then OUT_PARTICIPACAO_VENDA = Round((OUT_DEMANDA / OUT_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If NOV_VENDA_TOTAL <> 0 Then NOV_PARTICIPACAO_VENDA = Round((NOV_DEMANDA / NOV_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If DEZ_VENDA_TOTAL <> 0 Then DEZ_PARTICIPACAO_VENDA = Round((DEZ_DEMANDA / DEZ_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If YTD_VENDA_TOTAL <> 0 Then YTD_PARTICIPACAO_VENDA = Round((YTD_DEMANDA / YTD_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
            If TOTAL_VENDA_TOTAL <> 0 Then TOTAL_PARTICIPACAO_VENDA = Round((TOTAL_DEMANDA / TOTAL_VENDA_TOTAL) * 100, 2, MidpointRounding.AwayFromZero)
        ...
        sql_Dst = "SELECT DISTINCT GRUPO FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_TOTAL_FISCAL_UN_OR WHERE ANO = " & ANO & " AND GRUPO <> '01 - CSL BEHRING (Venda Direta)' AND GRUPO <> '(Venda Direta)' ORDER BY GRUPO "
        cmd_Dst.Connection = cnn_Dst
        cmd_Dst.CommandText = sql_Dst
        dtr_Dst = cmd_Dst.ExecuteReader
        ...
        Do While dtr_Dst.Read
            ...
            'RECUPERA VENDAS
            sql = ""
            sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_TOTAL_FISCAL_UN_OR WHERE GRUPO = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = 'VENDA UNIDADES ORIGINAL FISCAL' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "'"
            ...
            'RECUPERA DEMANDA
            sql = ""
            sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_TOTAL_FISCAL_UN_OR WHERE GRUPO = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = 'DEMANDA UNIDADES ORIGINAL FISCAL' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "'"
            ...
            If ANO = 2015 Then
                sql = ""
                sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_TOTAL_FISCAL_UN_OR WHERE GRUPO = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = 'ESTOQUE UNIDADES ORIGINAL FISCAL' AND ANO = " & ANO & " AND LINHA = '" & LINHA_PRODUTO & "'"
            Else
                sql = ""
                sql = "SELECT * FROM VIEW_DISTRIBUIDORES_MOVIMENTO_001_AGRUPADO_TOTAL_FISCAL_UN_OR WHERE GRUPO = '" & GRUPO_DISTRIBUIDOR & "' AND TIPO = 'ESTOQUE UNIDADES ORIGINAL FISCAL' AND ANO = " & ANO & " OR ANO = 2015 AND LINHA = '" & LINHA_PRODUTO & "'"
            End If
