...
Partial Class Produtos_Cadastro_Linha
    Inherits System.Web.UI.Page
    'Váriaveis Padrão
    Dim M As New clsMaster
    Dim Pagina As String = "Produtos_Cadastro_Linha.aspx"
    Dim Titulo As String = "Linhas de Produto - " & ConfigurationManager.AppSettings("NOME_SISTEMA").ToString
    ...
    Protected Sub cmd_Gravar_Click(sender As Object, e As EventArgs) Handles cmd_Gravar.Click
        If Len(txt_Linha.Text) = 0 Then
            Alert("Digite a linha de produto antes de gravar!", False, "")
            Exit Sub
        End If
        Sql = ""
        Sql = "Insert Into TBL_PRODUTOS_LINHAS (PRODUTO_LINHA, INCLUSAO_EMAIL, INCLUSAO_DATA) Values "
        Sql = Sql & "('" & M.ConverteTexto(txt_Linha.Text) & "', "
        ...
        If M.ExecutarSQL(Sql) = True Then
            ...
            dts_Lista.DataBind()
            gdv_Lista.DataBind()
            M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "Inclui linha de produto", txt_Linha.Text)
            Alert("Linha de Produto incluida com sucesso!", False, "")
        Else
            Mensagem = "ERRO - INCLUSAO LINHA DE PRODUTO - " & Pagina & " - " & Now() & Chr(10)
            Mensagem = Mensagem & "LINHA DE PRODUTO: " & txt_Linha.Text & Chr(10)
            Mensagem = Mensagem & "Por: " & Session("USUARIO_LOGIN") & " - " & Session("EMAIL_LOGIN")
            M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "ERRO", txt_Linha.Text)
            M.EnviarEmail("ERRO - INCLUSAO LINHA DE PRODUTO", Mensagem, True, False, "", "")
    ...
    Protected Sub gdv_Lista_RowUpdated(sender As Object, e As GridViewUpdatedEventArgs) Handles gdv_Lista.RowUpdated
        M.Inclui_Alteracao(gdv_Lista.SelectedValue, Session("EMAIL_LOGIN"), "COD_PRODUTO_LINHA", "TBL_PRODUTOS_LINHAS")
    End Sub
    Protected Sub gdv_Lista_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gdv_Lista.SelectedIndexChanged
        ...
        QTD_REGISTROS = M.Consiste_Exclusao(gdv_Lista.SelectedValue, "COD_PRODUTO_LINHA", "TBL_PRODUTOS")
        If QTD_REGISTROS > 0 Then
            Alert("Esta linha não pode ser excluido pois existem " & QTD_REGISTROS & " produtos relacionados", False, "")
        Else
            If M.ExecutarSQL("Delete From TBL_PRODUTOS_LINHAS Where COD_PRODUTO_LINHA = " & gdv_Lista.SelectedValue) = True Then
                M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "Excluiu Linha de produto COD", gdv_Lista.SelectedValue)
                Alert("Linha excluida com sucesso ", False, "")
                dts_Lista.DataBind()
                gdv_Lista.DataBind()
            End If
        End If
