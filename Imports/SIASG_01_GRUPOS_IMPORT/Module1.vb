...
Module Module1
    ...
    Public colunas_tabela As Integer = 2
                ...
                Using leitorArquivoCSV As New TextFieldParser(FI.FullName)
                    ...
                    While Not leitorArquivoCSV.EndOfData
                        ...
                        Try
                            ...
                            Next
                            strSql &= "'" & Formata_Texto(linhaAtual(0)) & "'," 'COD_GRUPO
                            strSql &= "'" & Formata_Texto(linhaAtual(1)) & "'," 'GRUPO
                            strSql &= "'" & FI.Name & "'," ' ARQUIVO
                            strSql &= "'" & Format(linha_arquivo, "000000") & "'," 'LINHA ARQUIVO
                            strSql &= "'" & Agora() & "'," 'ATUALIZACAO
                            strSql &= "'SUCESSO'," 'LOG
                            strSql &= 0 'CLASSES IMPORTAR
                            strSql &= ")"
                            strSql = Left(strSql, Len(strSql) - 1) & ")"


                            'VERIFICA SE A LINHA JÁ FOI IMPORTADA
                            Dim COD_GRUPO As String = Formata_Texto(linhaAtual(0))
                            Dim dtr_GRUPOS As Npgsql.NpgsqlDataReader = ExecuteSelect("SELECT [COD_GRUPO] FROM [" & schema & "]." & "[" & tabela & "] WHERE [COD_GRUPO] = '" & COD_GRUPO & "'")
                            If dtr_GRUPOS.HasRows Then
                                Mensagem("LINHA " & Format(linha_arquivo, "000000") & "; COD_GRUPO " & COD_GRUPO & "; JÁ INCLUS0")
                                GoTo PROXIMA_LINHA
                            End If

                            'CASO NÃO TENHA SIDO IMPORTA
                            If ExecuteSQL(strSql) = True Then Mensagem("LINHA;" & Format(linha_arquivo, "000000") & "; COD_GRUPO " & COD_GRUPO & ";OK")
