...
Module Module1
    ...
    Public tipo_arquivo As String = "ITENS"
    Public tipo_processo As String = "DOWNLOAD"
    Public mensagem_log As String
    'SIASG_08_PREGOES_ITENS_DOWNLOAD
    'VW_GHI_PREGOES_MEDITRONIC
    Sub Main()
        ...
        Try
            ...
            Select Case OPT_DOWNLOAD
                Case = "1"
                    sql = "SELECT * From [GHI].[VW_GHI_PREGOES_MEDITRONIC] Where [" & tipo_arquivo & "_OFFSET] < 0 And [ANO_EDITAL]::INT >= " & Val(OPT_ANO_EDITAL) & " And [" & tipo_arquivo & "_LOG] IS NULL "
                    Console.Title = "MEDTRONIC " & tipo_arquivo & " (NÃO IMPORTADAS, ANO INICIAL=" & OPT_ANO_EDITAL & ") " & tipo_processo & " Inicio " & Now()
                Case = "2"
                    sql = "SELECT * From [GHI].[VW_GHI_PREGOES_MEDITRONIC] Where [" & tipo_arquivo & "_OFFSET] < 0 And [ANO_EDITAL]::INT >= " & Val(OPT_ANO_EDITAL) & " And [" & tipo_arquivo & "_LOG] IS NOT NULL "
                ...
                Do While dtr.Read()
                    Do
                        Try
                            ...
                            Dim fileName As String = pasta_csv & "PREGOES_ITENS_" & dtr("COD_LICITACAO") & "_OFFSET_" & Format(offset, "0000000000") & ".CSV"
