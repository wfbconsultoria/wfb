...
Module Module1
    Public titulo_aplicacao As String = "ARGENTINA - PRODUTOS IMPORT"
    ...
    Public tabela As String = "TBL_PRODUTOS"
    Public strSql As String = ""
    Public erros As Integer = 0
    Public tentativas As Integer = 0
    Public produtos As Integer = 0
    Public mensagem_log As String


    Sub Main()
        Console.Title = "ARGENTINA - GET PRODUCT LIST - Inicio " & Now()
        Try
            Dim appPath As String = My.Application.Info.DirectoryPath
            Dim driver As IWebDriver
            driver = New ChromeDriver(appPath)
            ...
            sql &= " Where [ADJUDICADO] = 1 AND [PRODUTOS] = 0 AND [URL_PROCESSO]  IS NOT NULL ORDER BY [ANO] DESC, [MES] DESC "
            ...
            If dtr.HasRows Then
                ...
                Do While dtr.Read()
                    ...
                    URL_PROCESSO = dtr("URL_PROCESSO")
                    driver.Navigate().GoToUrl(URL_PROCESSO)

                    sql = "Update [ARGENTINA].[TBL_PROCESSOS] SET [PRODUTOS] = 0  Where [Número de Proceso] = '" & NUMERO_PROCESSO & "'"
                    ExecuteSQL(sql)
                    sql = "Delete From [ARGENTINA].[TBL_PRODUTOS] Where [Número de Proceso] = '" & NUMERO_PROCESSO & "'"
                    ExecuteSQL(sql)
...
STEP_table:
                        ...
                        table = wait.Until(ExpectedConditions.ElementExists(By.Id("ctl00_CPH1_UCVistaPreviaPliego_UC_DetalleProductos_gvLineaPliego")))
                    Catch
                        If tentativas > 10 Then GoTo PROXIMO_Processo
                        GoTo STEP_table
                    End Try

                    Dim rows = table.FindElements(By.TagName("tr"))
                    produtos = 0
                    For Each row In rows
                        Dim tds = row.FindElements(By.TagName("td"))
                        sql = ""
                        sql &= "Insert Into [ARGENTINA].[TBL_PRODUTOS] ("
                        sql &= "[Número de Proceso], [Número renglón], [Objeto del gasto], "
                        sql &= "[Código del ítem], [Descripción], [Cantidad], [Empty_01]) Values ('" & NUMERO_PROCESSO & "',"

                        For Each entry In tds
                            sql &= "'" & Formata_Texto(entry.Text) & "',"
                        Next
                        sql = Left(sql, Len(sql) - 1) & ")"
                        If ExecuteSQL(sql) = True Then produtos = produtos + 1
                    Next
                    sql = "Update [ARGENTINA].[TBL_PROCESSOS] SET [PRODUTOS] = " & produtos & " Where [Número de Proceso] = '" & NUMERO_PROCESSO & "'"
                    ExecuteSQL(sql)
PROXIMO_Processo:
                Loop
                Mensagem("SUCESSO - PRODUTOS RECUPERADOS COM SUCESSO")
            Else
