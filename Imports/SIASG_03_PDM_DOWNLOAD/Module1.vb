...
Module Module1
    ...
    Public tabela As String = "TB_CATMAT_CLASSES"
    Public pasta_csv As String = "D:\SIASG\03_PDM\ARQUIVOS\"
    Public erros As Integer = 0
    Public tipo_arquivo As String = "PDM"
    ...
    Sub Main()
        ...
        Try
            ...
            Dim sql As String = "SELECT * FROM [" & schema & "].[" & tabela & "] WHERE [" & tipo_arquivo & "_IMPORTAR] = 1"
                ...
                Do While dtr.Read()
                    Do
                        Try
                            ...
                            Dim fileName As String = pasta_csv & "PDM_CLASSE_" & Format(Val(dtr("Código")), "000000") & "_OFFSET_" & Format(offset, "0000000000") & ".CSV"
                            Dim myWebClient As New WebClient()

                            Console.WriteLine("SEARCH " & fileName)
                            ...
                            If fi_size > 0 Then
                                ...
                                ExecuteSQL("UPDATE [" & schema & "].[" & tabela & "] Set [" & tipo_arquivo & "_OFFSET] = " & offset & ", [" & tipo_arquivo & "_ATUALIZACAO] = '" & Agora() & "', [" & tipo_arquivo & "_LOG] = 'SUCESSO' WHERE [Código] = '" & dtr("Código") & "'")
                                offset += 500
                            Else
                                If offset = 0 Then
                                    Mensagem("EMPTY FILE OR NOT FOUND " & fileName)
                                    ExecuteSQL("UPDATE [" & schema & "].[" & tabela & "] Set [" & tipo_arquivo & "_OFFSET] = " & -1 & ", [" & tipo_arquivo & "_ATUALIZACAO] = '" & Agora() & "', [" & tipo_arquivo & "_LOG] = 'FALHA - EMPTY FILE OR NOT FOUND' WHERE [Código] = '" & dtr("Código") & "'")
                                    Exit Do
                                Else
                                    Exit Do
                                End If
                        ...
                        Catch exc2 As Exception
                            If offset = 0 Then
                                ExecuteSQL("UPDATE [" & schema & "].[" & tabela & "] Set [" & tipo_arquivo & "_OFFSET] = " & -1 & ", [" & tipo_arquivo & "_ATUALIZACAO] = '" & Agora() & "', [" & tipo_arquivo & "_LOG] = 'FALHA " & exc2.Message & "' WHERE [Código] = '" & dtr("Código") & "'")
                                LogErro(exc2)
                                Exit Do
                            Else
                                offset = 0
