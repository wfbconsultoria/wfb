...
Module Module1
    Public titulo_aplicacao As String = "SIASG - MATERIAIS IMPORT"
    ...
    Public colunas_tabela As Integer = 7
    ...
    Sub Main()
        Try
            ...
            For arquivos = 0 To FIs.Length - 1
                ...
                End If
                Mensagem("IMPORTANDO - " & Now() & "; " & FI.FullName)
                ...
                Using leitorArquivoCSV As New TextFieldParser(FI.FullName)
                    ...
                    While Not leitorArquivoCSV.EndOfData
                        ...
                        Try
                            ...
                            strSql &= "'" & Left(Formata_Texto(linhaAtual(4)), 5) & "',"
                            strSql &= "'" & Left(Formata_Texto(linhaAtual(3)), 4) & "',"
                            ...
                            strSql &= "'SUCESSO',"
                            importar = 1
                            If linhaAtual(5).ToString = "Ativo" Then importar = 1
                            strSql &= importar
                            strSql &= ")"

                            'VERIFICA SE A LINHA JÁ FOI IMPORTADA
                            Dim COD_MATERIAL As String = Formata_Texto(linhaAtual(0))
                            Dim dtr_MATERIAL As Npgsql.NpgsqlDataReader = ExecuteSelect("SELECT [COD_MATERIAL] FROM [" & schema & "]." & "[" & tabela & "] WHERE [COD_MATERIAL] = '" & COD_MATERIAL & "'")
                            If dtr_MATERIAL.HasRows Then
                                Mensagem("ARQUIVO " & Replace(FI.Name, ".CSV", "") & "; LINHA " & Format(linha_arquivo, "000000") & "; COD_PDM " & COD_MATERIAL & "; JÁ INCLUSA")
                                GoTo PROXIMA_LINHA
                            End If

                            'CASO NÃO TENHA SIDO IMPORTA
                            If ExecuteSQL(strSql) = True Then
                                Mensagem("ARQUIVO " & Replace(FI.Name, ".CSV", "") & "; LINHA;" & Format(linha_arquivo, "000000") & "; COD_MATERIAL " & COD_MATERIAL & "; OK;")
                            End If
