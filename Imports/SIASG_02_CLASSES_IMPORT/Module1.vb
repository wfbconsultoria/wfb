                ...
                Using leitorArquivoCSV As New TextFieldParser(FI.FullName)
                    ...
                    While Not leitorArquivoCSV.EndOfData
                        ...
                        Try
                            ...
                            Dim COD_CLASSE As String = Formata_Texto(linhaAtual(0))
                            Dim dtr_CLASSES As Npgsql.NpgsqlDataReader = ExecuteSelect("SELECT [COD_CLASSE] FROM [" & schema & "]." & "[" & tabela & "] WHERE [COD_CLASSE] = '" & COD_CLASSE & "'")
                            If dtr_CLASSES.HasRows Then
                                Mensagem("LINHA " & Format(linha_arquivo, "000000") & "; COD_CLASSE " & COD_CLASSE & "; JÁ INCLUSA")
                                GoTo PROXIMA_LINHA
                            End If

                            'CASO NÃO TENHA SIDO IMPORTA
                            If ExecuteSQL(strSql) = True Then
                                Mensagem("LINHA;" & Format(linha_arquivo, "000000") & "; COD_CLASSE " & COD_CLASSE & ";OK")
                            End If
