...
Module Module1
    Public titulo_aplicacao As String = "SIASG - PREGOES ITENS DOWNLOAD"
    Public cnnStr As String = "Server=bihospitalar.com.br;Port=5432;UserId=postgres;Password=@Mepm2412;Database=bih_datasus"
    ...
    Public pasta_csv As String = "S:\SIASG\PROPOSTAS\ARQUIVOS\"
    ...
    Sub Main()
        ...
        Try
            ...
            Dim sql As String = "SELECT * FROM [" & schema & "].[VW_PREGOES_ITENS_DOWNLOAD] WHERE [" & tipo_arquivo & "_IMPORTAR] = 1 AND [" & tipo_arquivo & "_OFFSET] < 0 "
            Dim dtr As Npgsql.NpgsqlDataReader = ExecuteSelect(sql)
            If dtr.HasRows Then
                Do While dtr.Read()
                    Do
                        Try
                            Dim remoteUri As String = Replace(dtr(tipo_arquivo & "_LINK"), "html", "csv") '& "&offset=" & offset
                            Dim fileName As String = pasta_csv & tipo_arquivo & "_" & dtr("COD_LICITACAO") & "_" & dtr("COD_ITEM") & ".CSV"
                            ...
                            If fi_size > 0 Then
                                ...
                                ExecuteSQL("UPDATE [" & schema & "].[" & tabela & "] Set [" & tipo_arquivo & "_OFFSET] = " & offset & ", [" & tipo_arquivo & "_ATUALIZACAO] = '" & Agora() & "', [" & tipo_arquivo & "_LOG] = 'SUCESSO' WHERE [COD_LICITACAO] = '" & dtr("COD_LICITACAO") & "' AND [COD_ITEM] = '" & dtr("COD_LICITACAO") & "'")
                                Exit Do
                                'offset = offset + 500
                            Else
                                If offset = 0 Then
                                    ...
                                    ExecuteSQL("UPDATE [" & schema & "].[" & tabela & "] Set [" & tipo_arquivo & "_OFFSET] = " & -1 & ", [" & tipo_arquivo & "_ATUALIZACAO] = '" & Agora() & "', [" & tipo_arquivo & "_LOG] = 'FALHA - EMPTY FILE OR NOT FOUND' WHERE [COD_LICITACAO] = '" & dtr("COD_LICITACAO") & "' AND [COD_ITEM] = '" & dtr("COD_LICITACAO") & "'")
                                    Exit Do
                                Else
                                    Exit Do
                                End If
                        ...
                        Catch exc2 As Exception
                            If offset = 0 Then
                                ExecuteSQL("UPDATE [" & schema & "].[" & tabela & "] Set [" & tipo_arquivo & "_OFFSET] = " & -1 & ", [" & tipo_arquivo & "_ATUALIZACAO] = '" & Agora() & "', [" & tipo_arquivo & "_LOG] = 'FALHA " & exc2.Message & "' WHERE [COD_LICITACAO] = '" & dtr("COD_LICITACAO") & "' AND [COD_ITEM] = '" & dtr("COD_LICITACAO") & "'")
                                LogErro(exc2)
                                Exit Do
                            Else
                                offset = 0
