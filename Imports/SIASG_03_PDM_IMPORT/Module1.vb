...
Module Module1
    ...
    Public colunas_tabela As Integer = 3
                ...
                Using leitorArquivoCSV As New TextFieldParser(FI.FullName)
                    ...
                    While Not leitorArquivoCSV.EndOfData
                        ...
                        Try
                            ...
                            strSql &= "[ATUALIZACAO],"
                            strSql &= "[LOG],"
                            strSql &= "[MATERIAIS_IMPORTAR]"
                            strSql &= ") Values ("
                            'colunas_arquivo = 0
                            'For Each campoAtual In linhaAtual
                            '    colunas_arquivo = colunas_arquivo + 1
                            '    strSql &= "'" & campoAtual & "',"
                            ...
                            strSql &= "'" & Left(Formata_Texto(linhaAtual(2)), 4) & "',"
                            ...
                            'VERIFICA SE A LINHA JÁ FOI IMPORTADA
                            Dim COD_PDM As String = Formata_Texto(linhaAtual(0))
                            Dim dtr_PDM As Npgsql.NpgsqlDataReader = ExecuteSelect("SELECT [COD_PDM] FROM [" & schema & "]." & "[" & tabela & "] WHERE [COD_PDM] = '" & COD_PDM & "'")
                            If dtr_PDM.HasRows Then
                                Mensagem("ARQUIVO " & Replace(FI.Name, ".CSV", "") & "; LINHA " & Format(linha_arquivo, "000000") & "; COD_PDM " & COD_PDM & "; JÁ INCLUSA")
                                GoTo PROXIMA_LINHA
                            End If

                            'CASO NÃO TENHA SIDO IMPORTA
                            If ExecuteSQL(strSql) = True Then
                                Mensagem("ARQUIVO " & Replace(FI.Name, ".CSV", "") & "; LINHA;" & Format(linha_arquivo, "000000") & "; COD_PDM " & COD_PDM & "; OK;")
                            End If
...
PROXIMA_LINHA:
    ...
    Public Sub LogErro(e As Exception, Optional mensagem_erro As String = "")
        ...
        If mensagem_erro = "" Then
            Mensagem("ARQUIVO " & Replace(FI.Name, ".CSV", "") & "; LINHA;" & Format(linha_arquivo, "000000") & ";ERRO;" & Replace(e.Message.ToString, vbCrLf, " "))
        Else
            Mensagem("ARQUIVO " & Replace(FI.Name, ".CSV", "") & ";LINHA;" & Format(linha_arquivo, "000000") & ";ERRO;" & Replace(e.Message.ToString, vbCrLf, " ") & ";" & mensagem_erro)
        End If
