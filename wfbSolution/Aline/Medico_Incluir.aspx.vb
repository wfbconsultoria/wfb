...
Partial Class Medico_Incluir
    ...
    Private Sub Medico_Incluir_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If dtr_medico_estabelecimento.HasRows Then
            ...
            'If dtr_medico_estabelecimento("ATIVO") = True Then ATIVO.Checked = True Else ATIVO.Checked = False
            'If dtr_medico_estabelecimento("ATIVO") = True Then ATIVO.Value = 1 Else ATIVO.Value = 0
            ID_MOTIVO_INATIVAR.Text = dtr_medico_estabelecimento("ID_MOTIVO_INATIVAR")
            JUSTIFICATIVA_INATIVAR.Value = dtr_medico_estabelecimento("JUSTIFICATIVA_INATIVAR").ToString
        End If
    ...
    Private Function Gravar() As Boolean
        ...
        Else
            ...
            'ZERA CAMPO DE INATIVACAO PARA INCLUSAO
            'ATIVO.Value = 1
            ID_MOTIVO_INATIVAR.Text = 0
            JUSTIFICATIVA_INATIVAR.Value = ""
        ...
        'If (ATIVO.Value = 1) Then bol_ATIVO = 1
        If ID_MOTIVO_INATIVAR.Text = 0 Then
            bol_ATIVO = 1
            JUSTIFICATIVA_INATIVAR.Value = ""
        Else
            bol_ATIVO = 0
        ...
        If dtr.HasRows Then
            ...
            sql &= " ATIVO = " & bol_ATIVO & ", "
            sql &= " ID_MOTIVO_INATIVAR = " & ID_MOTIVO_INATIVAR.Text & ", "
            sql &= " JUSTIFICATIVA_INATIVAR = '" & m.ConvertText(JUSTIFICATIVA_INATIVAR.Value, clsMaster.TextCaseOptions.UpperCase) & "', "
            sql &= " EMAIL_ALTERACAO = '" & m.ConvertText(Session("EMAIL_LOGIN"), clsMaster.TextCaseOptions.LowerCase) & "' "
        ...
        Else
            'INCLUI
            sql = ""
            sql &= " INSERT INTO [TBL_MEDICOS_ESTABELECIMENTOS] ([CRM_UF],[CNPJ],ID_FUNCAO,ATENDE_SEG,ATENDE_TER,ATENDE_QUA,ATENDE_QUI,ATENDE_SEX,ATIVO,ID_MOTIVO_INATIVAR,EMAIL_INCLUSAO) VALUES ( "
            sql &= " '" & strCRM_UF & "', "
            sql &= " '" & strCNPJ & "', "
            sql &= " '" & ID_FUNCAO.Text & "', "
            sql &= " '" & bol_ATENDE_SEG & "', "
...
Err_Gravar:
    ...
    Private Sub Atualiza_DTS()
        ...
        dts_MOTIVO_INATIVAR.SelectCommand = d.sql_motivo_inativar
        dts_MOTIVO_INATIVAR.DataBind()
        ...
        dts_MEDICOS.SelectCommand = "Select * from APP_MEDICOS_ESTABELECIMENTOS Where IdEstabelecimento  = '" & IdEstabelecimento & "' And ATIVO = 1 Order By NOME_SOBRENOME"
    ...
    Private Function validaCampos() As Boolean
        ...
        'If ATIVO.Value = 1 Then
        '    ID_MOTIVO_INATIVAR.Text = 0
        '    JUSTIFICATIVA_INATIVAR.Value = ""
        'Else
        '    If ID_MOTIVO_INATIVAR.Text = 0 Then m.Alert(Me, "Selecione o motivo da inativação", False, "")
        '    If Len(m.ConvertText(JUSTIFICATIVA_INATIVAR.Value)) = 0 Then m.Alert(Me, "Descreva a justificativa para inativar", False, "")
        '    validaCampos = False
        '    Exit Function
        'End If
    End Function
