Public Class Doctor
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        m.CheckLogin()
        m.CheckString(Request.QueryString("Doctor_Code"), "Doctors.aspx")

        RecoverRecord(Request.QueryString("Doctor_Code").ToString)

        'If Request.QueryString("Doctor_Code") = "NOVO" Then
        ...
        dtsDoctor.SelectCommand = "Select * From vw_Doctors Where Doctor_Code = '" & Request.QueryString("Doctor_Code") & "'"
        dtsDoctor.DataBind()
    ...
    Private Sub InsertRecord()
        ...
        If cmb_CRM_UF.Text = "00" Then
            m.Alert(Me, "SELECIONE A UF REFERENTE AO CRM")
            Exit Sub
        End If

        If d.FormatCRM(txt_CRM.Value) = "0000000" Then
            m.Alert(Me, "DIGITE O CRM")
            Exit Sub
        End If
        ...
        sql = sql & " '" & m.ConvertText(Session("Email"), clsMaster.TextCaseOptions.LowerCase) & "', "
        sql = sql & " '" & dt & "', "
        sql = sql & " '" & m.ConvertText(Session("Email"), clsMaster.TextCaseOptions.LowerCase) & "', "
        sql = sql & " '" & dt & "')"
        ...
        If m.ExecuteSQL(sql) = True Then
            m.Alert(Me, m.ConvertText(txt_Doctor_Name.Value, clsMaster.TextCaseOptions.UpperCase) & " INCLUIDO COM SUCESSO, Complete as informações de ENDEREÇO, DATA DE NASCIMENTO E CPF (opicional)", "Doctor?DoctorCode=" & Doctor_Code)
        End If
    End Sub
    Private Sub NewRecord()
        ...
        If Session("UserProfileCode") = "40" Then cmb_Account_Executive.SelectedValue = Session("Email")
    ...
    Private Sub SaveRecord()
        ...
        sql = sql & " [Update_User]= '" & m.ConvertText(Session("Email"), clsMaster.TextCaseOptions.UpperCase) & "', "
        sql = sql & " [Update_Date]= '" & m.GetDateTimeToString() & "' "
        sql = sql & " Where Doctor_Code = '" & Doctor_Code & "'"
        If m.ExecuteSQL(sql) = True Then
            m.Alert(Me, m.ConvertText(txt_Doctor_Name.Value, clsMaster.TextCaseOptions.UpperCase) & " ATUALIZADO COM SUCESSO !", "Doctor?DoctorCode=" & Doctor_Code)
        End If
    ...
    Sub ConsultaCPF()
        ...
        If txt_Birth_Date.Value = "" Then
            m.Alert(Me, "PARA VALIDAR O CPF É NECESSÁRIO A DATA DE NASCIMENTO")
            Exit Sub
        End If
        ...
        Try
            If pf.Status = False Then
                ...
                m.Alert(Me, m.ConvertText(txt_Doctor_Name.Value, clsMaster.TextCaseOptions.UpperCase) & " CPF OU DATA DE NASCIMENTO INVALIDOS")
                SaveRecord()
                Exit Sub
            End If
