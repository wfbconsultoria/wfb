<%@ Page Title="Serviços Check In/Out" Language="vb" AutoEventWireup="false" MasterPageFile="~/Master.Master" CodeBehind="CheckIn_List_Servicos.aspx.vb" Inherits="Chapeira_Eplanner.CheckIn_List_Servicos" %>
    ...
    <table class="table demo table-bordered table-hover " data-filter="#filter" data-paging="true" data-filter-text-only="true" data-page-size="1000">
        ...
        <tbody>
            <asp:Repeater ID="dtrTerceiros" runat="server" DataSourceID="dtsTerceiros">
                <ItemTemplate>
                    <tr>
                        <td class="text-primary" style="font-size: x-large; width: 75%"><%# DataBinder.Eval(Container.DataItem, "Nome").ToString%></td>
                        <td style="text-align: center; width: 25%"><a class='<%#"d-none d-md-block btn  btn-lg btn-" + DataBinder.Eval(Container.DataItem, "cor").ToString %>' href='<%# "CheckIn_Form.aspx?IdColaborador" + "=" + DataBinder.Eval(Container.DataItem, "ID").ToString %>'><%#DataBinder.Eval(Container.DataItem, "Acao").ToString %></a></td>
                </ItemTemplate>
            </asp:Repeater>
        </tbody>
        <tfoot>
