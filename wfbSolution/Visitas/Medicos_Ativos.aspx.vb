...
Partial Class Medicos_Ativos
    ...
    Protected Sub cmd_Gravar_Click(sender As Object, e As System.EventArgs) Handles cmd_Gravar.Click
        ...
        For Each gdvrow As GridViewRow In gdv_Localizar.Rows
            ATIVO = CType(gdvrow.Cells(7).FindControl("cmb_MEDICO_ATIVO"), DropDownList).SelectedValue()
            CNPJ = gdvrow.Cells(3).Text
            CRMUF = gdvrow.Cells(0).Text

            SQL = "SELECT CNPJ FROM TBL_MEDICOS_ESTABELECIMENTOS WHERE CNPJ = " & CNPJ
            cmd.Connection = cnn
            cmd.CommandText = SQL
            ...
            If dtr.HasRows Then
                ...
                SQL = SQL & " AND CRMUF = '" & CRMUF & "'"
            End If
            dtr.Close()
            If M.ExecutarSQL(SQL) = False Then
                M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "ERRO - ATUALIZAR SETORIZACAO", "CNPJ: " & CNPJ & " CRMUF: " & CRMUF)
                Mensagem = "ERRO  - ATUALIZAR MEDICO" & Chr(10)
                Mensagem = Mensagem & "CNPJ " & CNPJ & Chr(10)
                Mensagem = Mensagem & "ATIVO " & ATIVO & Chr(10)
                Mensagem = Mensagem & "CRMUF " & CRMUF & Chr(10)
    ...
    Protected Sub cmd_Excel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmd_Excel.Click
        ...
        Response.AddHeader("content-disposition", "attachment;filename=MEDICOS.xls")
        Response.Charset = ""
