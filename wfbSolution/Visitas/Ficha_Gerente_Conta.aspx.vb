Imports System.Math
Partial Class Ficha_Gerente_Conta
    Inherits System.Web.UI.Page
    Dim M As New clsMaster
    Dim Pagina As String = "Ficha_Gerente_Conta.aspx"
    ...
    Dim cnn_esfera As New System.Data.SqlClient.SqlConnection
    Dim cmd_esfera As New System.Data.SqlClient.SqlCommand
    Dim dtr_esfera As System.Data.SqlClient.SqlDataReader
    Dim sql_esfera As String

    Dim cnn_est As New System.Data.SqlClient.SqlConnection
    Dim cmd_est As New System.Data.SqlClient.SqlCommand
    Dim dtr_est As System.Data.SqlClient.SqlDataReader
    Dim sql_est As String
    ...
    Dim ESFERA_ADMINISTRATIVA As String
    ...
    Dim MEDIA_DEMANDA_RETRASADO As Double
    ...
    Dim MEDIA_DEMANDA_PASSADO As Double
    ...
    Dim MEDIA_DEMANDA_ATUAL As Double

    Dim JAN_DEMANDA_ATUAL_PUBLICO As Double
    Dim FEV_DEMANDA_ATUAL_PUBLICO As Double
    Dim MAR_DEMANDA_ATUAL_PUBLICO As Double
    Dim ABR_DEMANDA_ATUAL_PUBLICO As Double
    Dim MAI_DEMANDA_ATUAL_PUBLICO As Double
    Dim JUN_DEMANDA_ATUAL_PUBLICO As Double
    Dim JUL_DEMANDA_ATUAL_PUBLICO As Double
    Dim AGO_DEMANDA_ATUAL_PUBLICO As Double
    Dim SET_DEMANDA_ATUAL_PUBLICO As Double
    Dim OUT_DEMANDA_ATUAL_PUBLICO As Double
    Dim NOV_DEMANDA_ATUAL_PUBLICO As Double
    Dim DEZ_DEMANDA_ATUAL_PUBLICO As Double
    Dim TRI01_DEMANDA_ATUAL_PUBLICO As Double
    Dim TRI02_DEMANDA_ATUAL_PUBLICO As Double
    Dim TRI03_DEMANDA_ATUAL_PUBLICO As Double
    Dim TRI04_DEMANDA_ATUAL_PUBLICO As Double
    Dim TOTAL_DEMANDA_ATUAL_PUBLICO As Double
    Dim YTD_DEMANDA_ATUAL_PUBLICO As Double
    Dim MEDIA_DEMANDA_ATUAL_PUBLICO As Double

    Dim JAN_DEMANDA_ATUAL_PRIVADO As Double
    Dim FEV_DEMANDA_ATUAL_PRIVADO As Double
    Dim MAR_DEMANDA_ATUAL_PRIVADO As Double
    Dim ABR_DEMANDA_ATUAL_PRIVADO As Double
    Dim MAI_DEMANDA_ATUAL_PRIVADO As Double
    Dim JUN_DEMANDA_ATUAL_PRIVADO As Double
    Dim JUL_DEMANDA_ATUAL_PRIVADO As Double
    Dim AGO_DEMANDA_ATUAL_PRIVADO As Double
    Dim SET_DEMANDA_ATUAL_PRIVADO As Double
    Dim OUT_DEMANDA_ATUAL_PRIVADO As Double
    Dim NOV_DEMANDA_ATUAL_PRIVADO As Double
    Dim DEZ_DEMANDA_ATUAL_PRIVADO As Double
    Dim TRI01_DEMANDA_ATUAL_PRIVADO As Double
    Dim TRI02_DEMANDA_ATUAL_PRIVADO As Double
    Dim TRI03_DEMANDA_ATUAL_PRIVADO As Double
    Dim TRI04_DEMANDA_ATUAL_PRIVADO As Double
    Dim TOTAL_DEMANDA_ATUAL_PRIVADO As Double
    Dim YTD_DEMANDA_ATUAL_PRIVADO As Double
    Dim MEDIA_DEMANDA_ATUAL_PRIVADO As Double
    ...
    Dim MEDIA_BASE_NEGOCIOS As Double
    ...
    Dim MEDIA_OPORTUNIDADES_025 As Double
    ...
    Dim MEDIA_OPORTUNIDADES_050 As Double
    ...
    Dim MEDIA_OPORTUNIDADES_075 As Double
    ...
    Dim MEDIA_OPORTUNIDADES_100 As Double
    ...
    Dim MEDIA_OPORTUNIDADES_TOTAL As Double
    ...
    Dim MEDIA_CRESCIMENTO_ATUAL_PASSADO As Double
    ...
    Dim MEDIA_CRESCIMENTO_PASSADO_RETRASADO As Double
    ...
    Dim MEDIA_PERFORMANCE As Double
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("COD_PERFIL_LOGIN") = "0" Then sql = "Select '@' as EMAIL, '@ Selecione' as NOME UNION ALL  Select  EMAIL, NOME From TBL_USUARIOS Where COD_PERFIL = '3' ORDER BY NOME"
        If Session("COD_PERFIL_LOGIN") = "1" Then sql = "Select '@' as EMAIL, '@ Selecione' as NOME UNION ALL  Select  EMAIL, NOME From TBL_USUARIOS Where COD_PERFIL = '3' ORDER BY NOME"
        If Session("COD_PERFIL_LOGIN") = "2" Then Alert("Você não tem acesso a esta página", True, "Default.aspx")
        If Session("COD_PERFIL_LOGIN") = "3" Then sql = "Select '@' as EMAIL, '@ Selecione' as NOME UNION ALL Select  EMAIL, NOME From TBL_USUARIOS Where  EMAIL = '" & Session("EMAIL_LOGIN") & "' ORDER BY NOME"
        ...
        cnn_est.ConnectionString = ConfigurationManager.ConnectionStrings("cnnStr").ConnectionString
        cnn_est.Open()
        cnn_esfera.ConnectionString = ConfigurationManager.ConnectionStrings("cnnStr").ConnectionString
        cnn_esfera.Open()
        ...
        If dtr_linha_produto.HasRows Then
            ...
            Do While dtr_linha_produto.Read
                ...
                'MONTA RELATORIO
                Linha_Vazia(30, "Demanda", True)
                Linha_Cabecalho_Meses("")
                ...
                Linha_Demanda_Atual_Publico(dtr_linha_produto("LINHA"), "Público", True)
                Linha_Demanda_Atual_Privado(dtr_linha_produto("LINHA"), "Privado", True)

                Linha_Vazia(30, "Crescimento %", True)
                Linha_Cabecalho_Meses("")
                Linha_Demanda_Retrasado(dtr_linha_produto("LINHA"), ANO_RETRASADO, False)
                Linha_Demanda_Passado(dtr_linha_produto("LINHA"), ANO_PASSADO, False)
                Linha_Demanda_Atual(dtr_linha_produto("LINHA"), ANO_ATUAL, False)
                ...
                Linha_Vazia(30, "Funil de Vendas " & ANO_ATUAL, True)
                Linha_Cabecalho_Meses("")
                Linha_Base_Negocios(dtr_linha_produto("LINHA"), "Base Negócios", True)
                Linha_OPORTUNIDADES_TOTAL(dtr_linha_produto("LINHA"), "Oportunidades Total", True)
                ...
                Linha_Vazia(30, "Perfomance " & ANO_ATUAL, True)
                Linha_Cabecalho_Meses("TOTAL")
                ...
                '    'MONTA DEMANDA DOS ESTABELECIMENTOS 
                '    sql_est = "Select Distinct CNPJ, ESTABELECIMENTO_CNPJ, TOTAL FROM VIEW_DEMANDA_001_DETALHADO WHERE ANO = " & ANO_ATUAL & " AND EMAIL_REPRESENTANTE = '" & cmb_Usuario.Text & "' ORDER BY TOTAL DESC"
                '    cmd_est.Connection = cnn_est
                '    cmd_est.CommandText = sql_est
                '    dtr_est = cmd_est.ExecuteReader

                '    Linha_Vazia(5, "", True)
                '    Linha_Cabecalho_Meses("")
                '    While dtr_est.Read()
                '        CNPJ = dtr_est("CNPJ")
                '        ESTABELECIMENTO = dtr_est("ESTABELECIMENTO_CNPJ")
                '        Linha_Demanda_Atual_EST(dtr_linha_produto("LINHA"), ESTABELECIMENTO, True)
                '    End While
                '    dtr_est.Close()
            Loop
    ...
    Private Sub Linha_Demanda_Retrasado(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "WHERE (ANO = " & ANO_RETRASADO & " AND LINHA = '" & LINHA_PRODUTO & "' AND EMAIL_GERENTE_CONTA = '" & cmb_Usuario.Text & "')"
        ...
        If dtr.HasRows Then
            ...
            If Not (IsDBNull(dtr("YTD"))) Then YTD_DEMANDA_RETRASADO = dtr("YTD")
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_DEMANDA_RETRASADO = dtr("TOTAL")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_DEMANDA_RETRASADO = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_DEMANDA_RETRASADO = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_DEMANDA_RETRASADO = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_DEMANDA_RETRASADO = dtr("TRI04")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_DEMANDA_RETRASADO = dtr("MEDIA")
        End If
        dtr.Close()
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_DEMANDA_RETRASADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
    ...
    Private Sub Linha_Demanda_Passado(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "SUM(MEDIA) AS MEDIA, SUM(TOTAL) AS TOTAL, SUM(TRI01) AS TRI01, SUM(TRI02) AS TRI02,SUM(TRI03) AS TRI03,SUM(TRI04) AS TRI04,SUM(YTD) AS YTD "
        sql = sql & "FROM dbo.VIEW_DEMANDA_001_DETALHADO "
        sql = sql & "WHERE (ANO = " & ANO_PASSADO & " AND LINHA = '" & LINHA_PRODUTO & "' AND EMAIL_GERENTE_CONTA = '" & cmb_Usuario.Text & "')"
        ...
        If dtr.HasRows Then
            ...
            If Not (IsDBNull(dtr("YTD"))) Then YTD_DEMANDA_PASSADO = dtr("YTD")
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_DEMANDA_PASSADO = dtr("TOTAL")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_DEMANDA_PASSADO = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_DEMANDA_PASSADO = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_DEMANDA_PASSADO = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_DEMANDA_PASSADO = dtr("TRI04")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_DEMANDA_PASSADO = dtr("MEDIA")
        End If
        dtr.Close()
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_DEMANDA_PASSADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
    ...
    Private Sub Linha_Demanda_Atual(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "SUM(MEDIA) AS MEDIA, SUM(TOTAL) AS TOTAL, SUM(TRI01) AS TRI01, SUM(TRI02) AS TRI02,SUM(TRI03) AS TRI03,SUM(TRI04) AS TRI04,SUM(YTD) AS YTD "
        sql = sql & "FROM dbo.VIEW_DEMANDA_001_DETALHADO "
        sql = sql & "WHERE (ANO = " & ANO_ATUAL & " AND LINHA = '" & LINHA_PRODUTO & "' AND EMAIL_GERENTE_CONTA = '" & cmb_Usuario.Text & "')"
        ...
        If dtr.HasRows Then
            ...
            If Not (IsDBNull(dtr("YTD"))) Then YTD_DEMANDA_ATUAL = dtr("YTD")
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_DEMANDA_ATUAL = dtr("TOTAL")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_DEMANDA_ATUAL = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_DEMANDA_ATUAL = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_DEMANDA_ATUAL = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_DEMANDA_ATUAL = dtr("TRI04")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_DEMANDA_ATUAL = dtr("MEDIA")
        End If
        dtr.Close()
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_DEMANDA_ATUAL, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
    ...
    End Sub
    Private Sub Linha_Demanda_Atual_Publico(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        'RECUPERA INFORMAÇÕES
        JAN_DEMANDA_ATUAL_PUBLICO = 0
        FEV_DEMANDA_ATUAL_PUBLICO = 0
        MAR_DEMANDA_ATUAL_PUBLICO = 0
        ABR_DEMANDA_ATUAL_PUBLICO = 0
        MAI_DEMANDA_ATUAL_PUBLICO = 0
        JUN_DEMANDA_ATUAL_PUBLICO = 0
        JUL_DEMANDA_ATUAL_PUBLICO = 0
        AGO_DEMANDA_ATUAL_PUBLICO = 0
        SET_DEMANDA_ATUAL_PUBLICO = 0
        OUT_DEMANDA_ATUAL_PUBLICO = 0
        NOV_DEMANDA_ATUAL_PUBLICO = 0
        DEZ_DEMANDA_ATUAL_PUBLICO = 0
        TRI01_DEMANDA_ATUAL_PUBLICO = 0
        TRI02_DEMANDA_ATUAL_PUBLICO = 0
        TRI03_DEMANDA_ATUAL_PUBLICO = 0
        TRI04_DEMANDA_ATUAL_PUBLICO = 0
        TOTAL_DEMANDA_ATUAL_PUBLICO = 0
        YTD_DEMANDA_ATUAL_PUBLICO = 0
        MEDIA_DEMANDA_ATUAL_PUBLICO = 0
        ...
        sql = sql & "SUM(MEDIA) AS MEDIA, SUM(TOTAL) AS TOTAL, SUM(TRI01) AS TRI01, SUM(TRI02) AS TRI02,SUM(TRI03) AS TRI03,SUM(TRI04) AS TRI04,SUM(YTD) AS YTD "
        sql = sql & "FROM dbo.VIEW_DEMANDA_001_DETALHADO "
        sql = sql & "WHERE (ESFERA = 'PUBLICO' AND ANO = " & ANO_ATUAL & " AND LINHA = '" & LINHA_PRODUTO & "' AND EMAIL_GERENTE_CONTA = '" & cmb_Usuario.Text & "')"
        ...
        dtr.Read()
        If dtr.HasRows Then
            If Not (IsDBNull(dtr("JAN"))) Then JAN_DEMANDA_ATUAL_PUBLICO = dtr("JAN")
            If Not (IsDBNull(dtr("FEV"))) Then FEV_DEMANDA_ATUAL_PUBLICO = dtr("FEV")
            If Not (IsDBNull(dtr("MAR"))) Then MAR_DEMANDA_ATUAL_PUBLICO = dtr("MAR")
            If Not (IsDBNull(dtr("ABR"))) Then ABR_DEMANDA_ATUAL_PUBLICO = dtr("ABR")
            If Not (IsDBNull(dtr("MAI"))) Then MAI_DEMANDA_ATUAL_PUBLICO = dtr("MAI")
            If Not (IsDBNull(dtr("JUN"))) Then JUN_DEMANDA_ATUAL_PUBLICO = dtr("JUN")
            If Not (IsDBNull(dtr("JUL"))) Then JUL_DEMANDA_ATUAL_PUBLICO = dtr("JUL")
            If Not (IsDBNull(dtr("AGO"))) Then AGO_DEMANDA_ATUAL_PUBLICO = dtr("AGO")
            If Not (IsDBNull(dtr("SET"))) Then SET_DEMANDA_ATUAL_PUBLICO = dtr("SET")
            If Not (IsDBNull(dtr("OUT"))) Then OUT_DEMANDA_ATUAL_PUBLICO = dtr("OUT")
            If Not (IsDBNull(dtr("NOV"))) Then NOV_DEMANDA_ATUAL_PUBLICO = dtr("NOV")
            If Not (IsDBNull(dtr("DEZ"))) Then DEZ_DEMANDA_ATUAL_PUBLICO = dtr("DEZ")
            If Not (IsDBNull(dtr("YTD"))) Then YTD_DEMANDA_ATUAL_PUBLICO = dtr("YTD")
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_DEMANDA_ATUAL_PUBLICO = dtr("TOTAL")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_DEMANDA_ATUAL_PUBLICO = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_DEMANDA_ATUAL_PUBLICO = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_DEMANDA_ATUAL_PUBLICO = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_DEMANDA_ATUAL_PUBLICO = dtr("TRI04")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_DEMANDA_ATUAL_PUBLICO = dtr("MEDIA")
        End If
        dtr.Close()
        ...
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green

        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(JAN_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(FEV_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MAR_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(ABR_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MAI_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(JUN_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(JUL_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(AGO_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(SET_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(OUT_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(NOV_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(DEZ_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(TRI01_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(TRI02_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(TRI03_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(TRI04_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(YTD_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(TOTAL_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_DEMANDA_ATUAL_PUBLICO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Green
    ...
    End Sub
    Private Sub Linha_Demanda_Atual_Privado(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        'RECUPERA INFORMAÇÕES
        JAN_DEMANDA_ATUAL_PRIVADO = 0
        FEV_DEMANDA_ATUAL_PRIVADO = 0
        MAR_DEMANDA_ATUAL_PRIVADO = 0
        ABR_DEMANDA_ATUAL_PRIVADO = 0
        MAI_DEMANDA_ATUAL_PRIVADO = 0
        JUN_DEMANDA_ATUAL_PRIVADO = 0
        JUL_DEMANDA_ATUAL_PRIVADO = 0
        AGO_DEMANDA_ATUAL_PRIVADO = 0
        SET_DEMANDA_ATUAL_PRIVADO = 0
        OUT_DEMANDA_ATUAL_PRIVADO = 0
        NOV_DEMANDA_ATUAL_PRIVADO = 0
        DEZ_DEMANDA_ATUAL_PRIVADO = 0
        TRI01_DEMANDA_ATUAL_PRIVADO = 0
        TRI02_DEMANDA_ATUAL_PRIVADO = 0
        TRI03_DEMANDA_ATUAL_PRIVADO = 0
        TRI04_DEMANDA_ATUAL_PRIVADO = 0
        TOTAL_DEMANDA_ATUAL_PRIVADO = 0
        YTD_DEMANDA_ATUAL_PRIVADO = 0
        MEDIA_DEMANDA_ATUAL_PRIVADO = 0
        ...
        dtr.Read()
        If dtr.HasRows Then
            If Not (IsDBNull(dtr("JAN"))) Then JAN_DEMANDA_ATUAL_PRIVADO = dtr("JAN")
            If Not (IsDBNull(dtr("FEV"))) Then FEV_DEMANDA_ATUAL_PRIVADO = dtr("FEV")
            If Not (IsDBNull(dtr("MAR"))) Then MAR_DEMANDA_ATUAL_PRIVADO = dtr("MAR")
            If Not (IsDBNull(dtr("ABR"))) Then ABR_DEMANDA_ATUAL_PRIVADO = dtr("ABR")
            If Not (IsDBNull(dtr("MAI"))) Then MAI_DEMANDA_ATUAL_PRIVADO = dtr("MAI")
            If Not (IsDBNull(dtr("JUN"))) Then JUN_DEMANDA_ATUAL_PRIVADO = dtr("JUN")
            If Not (IsDBNull(dtr("JUL"))) Then JUL_DEMANDA_ATUAL_PRIVADO = dtr("JUL")
            If Not (IsDBNull(dtr("AGO"))) Then AGO_DEMANDA_ATUAL_PRIVADO = dtr("AGO")
            If Not (IsDBNull(dtr("SET"))) Then SET_DEMANDA_ATUAL_PRIVADO = dtr("SET")
            If Not (IsDBNull(dtr("OUT"))) Then OUT_DEMANDA_ATUAL_PRIVADO = dtr("OUT")
            If Not (IsDBNull(dtr("NOV"))) Then NOV_DEMANDA_ATUAL_PRIVADO = dtr("NOV")
            If Not (IsDBNull(dtr("DEZ"))) Then DEZ_DEMANDA_ATUAL_PRIVADO = dtr("DEZ")
            If Not (IsDBNull(dtr("YTD"))) Then YTD_DEMANDA_ATUAL_PRIVADO = dtr("YTD")
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_DEMANDA_ATUAL_PRIVADO = dtr("TOTAL")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_DEMANDA_ATUAL_PRIVADO = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_DEMANDA_ATUAL_PRIVADO = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_DEMANDA_ATUAL_PRIVADO = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_DEMANDA_ATUAL_PRIVADO = dtr("TRI04")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_DEMANDA_ATUAL_PRIVADO = dtr("MEDIA")
        End If
        dtr.Close()
        ...
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue

        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(JAN_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(FEV_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MAR_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(ABR_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MAI_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(JUN_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(JUL_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(AGO_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(SET_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(OUT_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(NOV_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(DEZ_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(TRI01_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(TRI02_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(TRI03_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(TRI04_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(YTD_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(TOTAL_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_DEMANDA_ATUAL_PRIVADO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        cel.ForeColor = Drawing.Color.Blue
    ...
    Private Sub Linha_Base_Negocios(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        If dtr.HasRows Then
            ...
            If Not (IsDBNull(dtr("YTD"))) Then YTD_BASE_NEGOCIOS = dtr("YTD")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_BASE_NEGOCIOS = dtr("MEDIA")
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_BASE_NEGOCIOS = dtr("TOTAL")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_BASE_NEGOCIOS = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_BASE_NEGOCIOS = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_BASE_NEGOCIOS = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_BASE_NEGOCIOS = dtr("TRI04")
        End If
        dtr.Close()
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_BASE_NEGOCIOS, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
    ...
    Private Sub Linha_Oportunidades_025(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        MEDIA_OPORTUNIDADES_025 = 0

        sql = ""
        sql = sql & "SELECT SUM(JAN) AS JAN, SUM(FEV) AS FEV, SUM(MAR) AS MAR, SUM(ABR) AS ABR, SUM(MAI) AS MAI, SUM(JUN) AS JUN, SUM(JUL) AS JUL, "
        sql = sql & "SUM(AGO) AS AGO, SUM([SET]) AS [SET], SUM(OUT) AS OUT, SUM(NOV) AS NOV, SUM(DEZ) AS DEZ, "
        sql = sql & "SUM(MEDIA) AS MEDIA, SUM(TOTAL) AS TOTAL, SUM(TRI01) AS TRI01, SUM(TRI02) AS TRI02,SUM(TRI03) AS TRI03,SUM(TRI04) AS TRI04,SUM(YTD) AS YTD "
        sql = sql & "FROM dbo.VIEW_OPORTUNIDADES_001_DETALHADO "
        sql = sql & "WHERE (ANO_CONCLUSAO_PREVISTO = " & ANO_ATUAL & ") AND ID_FASE_ATUAL = 2 AND ATIVA = 'True' AND LINHA_PRODUTO = '" & LINHA_PRODUTO & "' AND EMAIL_GERENTE_CONTA = '" & cmb_Usuario.Text & "'"
        ...
        If dtr.HasRows Then
            ...
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_OPORTUNIDADES_025 = dtr("TOTAL")
            If Not (IsDBNull(dtr("YTD"))) Then YTD_OPORTUNIDADES_025 = dtr("YTD")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_OPORTUNIDADES_025 = dtr("MEDIA")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_OPORTUNIDADES_025 = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_OPORTUNIDADES_025 = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_OPORTUNIDADES_025 = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_OPORTUNIDADES_025 = dtr("TRI04")
        End If
        dtr.Close()
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_OPORTUNIDADES_025, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
    ...
    Private Sub Linha_Oportunidades_050(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "SUM(MEDIA) AS MEDIA, SUM(TOTAL) AS TOTAL, SUM(TRI01) AS TRI01, SUM(TRI02) AS TRI02,SUM(TRI03) AS TRI03,SUM(TRI04) AS TRI04,SUM(YTD) AS YTD "
        sql = sql & "FROM dbo.VIEW_OPORTUNIDADES_001_DETALHADO "
        sql = sql & "WHERE (ANO_CONCLUSAO_PREVISTO = " & ANO_ATUAL & ") AND ID_FASE_ATUAL = 3 AND ATIVA = 'True' AND LINHA_PRODUTO = '" & LINHA_PRODUTO & "' AND EMAIL_GERENTE_CONTA = '" & cmb_Usuario.Text & "'"
        ...
        If dtr.HasRows Then
            ...
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_OPORTUNIDADES_050 = dtr("TOTAL")
            If Not (IsDBNull(dtr("YTD"))) Then YTD_OPORTUNIDADES_050 = dtr("YTD")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_OPORTUNIDADES_050 = dtr("MEDIA")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_OPORTUNIDADES_050 = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_OPORTUNIDADES_050 = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_OPORTUNIDADES_050 = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_OPORTUNIDADES_050 = dtr("TRI04")
        End If
        dtr.Close()
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_OPORTUNIDADES_050, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
    ...
    Private Sub Linha_Oportunidades_075(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "SUM(MEDIA) AS MEDIA, SUM(TOTAL) AS TOTAL, SUM(TRI01) AS TRI01, SUM(TRI02) AS TRI02,SUM(TRI03) AS TRI03,SUM(TRI04) AS TRI04,SUM(YTD) AS YTD "
        sql = sql & "FROM dbo.VIEW_OPORTUNIDADES_001_DETALHADO "
        sql = sql & "WHERE (ANO_CONCLUSAO_PREVISTO = " & ANO_ATUAL & ") AND ID_FASE_ATUAL = 4 AND ATIVA = 'True' AND LINHA_PRODUTO = '" & LINHA_PRODUTO & "' AND EMAIL_GERENTE_CONTA = '" & cmb_Usuario.Text & "'"
        ...
        If dtr.HasRows Then
            ...
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_OPORTUNIDADES_075 = dtr("TOTAL")
            If Not (IsDBNull(dtr("YTD"))) Then YTD_OPORTUNIDADES_075 = dtr("YTD")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_OPORTUNIDADES_075 = dtr("MEDIA")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_OPORTUNIDADES_075 = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_OPORTUNIDADES_075 = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_OPORTUNIDADES_075 = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_OPORTUNIDADES_075 = dtr("TRI04")
        End If
        dtr.Close()
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_OPORTUNIDADES_075, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
    ...
    Private Sub Linha_OPORTUNIDADES_100(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "SUM(MEDIA) AS MEDIA, SUM(TOTAL) AS TOTAL, SUM(TRI01) AS TRI01, SUM(TRI02) AS TRI02,SUM(TRI03) AS TRI03,SUM(TRI04) AS TRI04,SUM(YTD) AS YTD "
        sql = sql & "FROM dbo.VIEW_OPORTUNIDADES_001_DETALHADO "
        sql = sql & "WHERE (ANO_CONCLUSAO_PREVISTO = " & ANO_ATUAL & ") AND ID_FASE_ATUAL = 5 AND ATIVA = 'True' AND LINHA_PRODUTO = '" & LINHA_PRODUTO & "' AND EMAIL_GERENTE_CONTA = '" & cmb_Usuario.Text & "'"
        ...
        If dtr.HasRows Then
            ...
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_OPORTUNIDADES_100 = dtr("TOTAL")
            If Not (IsDBNull(dtr("YTD"))) Then YTD_OPORTUNIDADES_100 = dtr("YTD")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_OPORTUNIDADES_100 = dtr("MEDIA")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_OPORTUNIDADES_100 = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_OPORTUNIDADES_100 = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_OPORTUNIDADES_100 = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_OPORTUNIDADES_100 = dtr("TRI04")
        End If
        dtr.Close()
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_OPORTUNIDADES_100, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
    ...
    Private Sub Linha_OPORTUNIDADES_TOTAL(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "SUM(MEDIA) AS MEDIA, SUM(TOTAL) AS TOTAL, SUM(TRI01) AS TRI01, SUM(TRI02) AS TRI02,SUM(TRI03) AS TRI03,SUM(TRI04) AS TRI04,SUM(YTD) AS YTD "
        sql = sql & "FROM dbo.VIEW_OPORTUNIDADES_001_DETALHADO "
        sql = sql & "WHERE (ANO_CONCLUSAO_PREVISTO = " & ANO_ATUAL & ")  AND ATIVA = 'True' AND LINHA_PRODUTO = '" & LINHA_PRODUTO & "' AND EMAIL_GERENTE_CONTA = '" & cmb_Usuario.Text & "'"
        ...
        If dtr.HasRows Then
            ...
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_OPORTUNIDADES_TOTAL = dtr("TOTAL")
            If Not (IsDBNull(dtr("YTD"))) Then YTD_OPORTUNIDADES_TOTAL = dtr("YTD")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_OPORTUNIDADES_TOTAL = dtr("MEDIA")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_OPORTUNIDADES_TOTAL = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_OPORTUNIDADES_TOTAL = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_OPORTUNIDADES_TOTAL = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_OPORTUNIDADES_TOTAL = dtr("TRI04")
        End If
        dtr.Close()
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_OPORTUNIDADES_TOTAL, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
    ...
    Private Sub Linha_PREVISTO(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        MEDIA_PREVISTO = TOTAL_PREVISTO / 12
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_PREVISTO, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
    ...
    Private Sub Linha_CRESCIMENTO_ATUAL_PASSADO(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        MEDIA_CRESCIMENTO_ATUAL_PASSADO = 0
        ...
        If MEDIA_DEMANDA_ATUAL = 0 Then
            MEDIA_CRESCIMENTO_ATUAL_PASSADO = 0
        Else
            If MEDIA_DEMANDA_PASSADO = 0 Then
                MEDIA_CRESCIMENTO_ATUAL_PASSADO = 100
            Else
                MEDIA_CRESCIMENTO_ATUAL_PASSADO = ((MEDIA_DEMANDA_ATUAL / MEDIA_DEMANDA_PASSADO) - 1) * 100
            End If
        End If
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_CRESCIMENTO_ATUAL_PASSADO, 2) & " %"
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        If MEDIA_CRESCIMENTO_ATUAL_PASSADO < 80 Then cel.BackColor = Drawing.Color.White
        If MEDIA_CRESCIMENTO_ATUAL_PASSADO <= 0 Then cel.BackColor = Drawing.Color.FromArgb(255, 100, 100)
        If MEDIA_CRESCIMENTO_ATUAL_PASSADO > 30 Then cel.BackColor = Drawing.Color.LightSkyBlue
    ...
    Private Sub Linha_CRESCIMENTO_PASSADO_RETRASADO(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        MEDIA_CRESCIMENTO_PASSADO_RETRASADO = 0
        ...
        If MEDIA_DEMANDA_PASSADO = 0 Then
            MEDIA_CRESCIMENTO_PASSADO_RETRASADO = 0
        Else
            If MEDIA_DEMANDA_RETRASADO = 0 Then
                MEDIA_CRESCIMENTO_PASSADO_RETRASADO = 100
            Else
                MEDIA_CRESCIMENTO_PASSADO_RETRASADO = ((MEDIA_DEMANDA_PASSADO / MEDIA_DEMANDA_RETRASADO) - 1) * 100
            End If
        End If
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_CRESCIMENTO_PASSADO_RETRASADO, 2) & " %"
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
        If MEDIA_CRESCIMENTO_PASSADO_RETRASADO < 80 Then cel.BackColor = Drawing.Color.White
        If MEDIA_CRESCIMENTO_PASSADO_RETRASADO <= 0 Then cel.BackColor = Drawing.Color.FromArgb(255, 100, 100)
        If MEDIA_CRESCIMENTO_PASSADO_RETRASADO > 30 Then cel.BackColor = Drawing.Color.LightSkyBlue
    ...
    Private Sub Linha_Cota_Atual(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "SELECT SUM(JAN) AS JAN, SUM(FEV) AS FEV, SUM(MAR) AS MAR, SUM(ABR) AS ABR, SUM(MAI) AS MAI, SUM(JUN) AS JUN, SUM(JUL) AS JUL, "
        sql = sql & "SUM(AGO) AS AGO, SUM([SET]) AS [SET], SUM(OUT) AS OUT, SUM(NOV) AS NOV, SUM(DEZ) AS DEZ, "
        sql = sql & "SUM(MEDIA) AS MEDIA, SUM(TOTAL) AS TOTAL, SUM(TRI1) AS TRI01, SUM(TRI2) AS TRI02,SUM(TRI3) AS TRI03,SUM(TRI4) AS TRI04,SUM(YTD) AS YTD "
        sql = sql & "FROM dbo.VIEW_COTAS "
        sql = sql & "WHERE (ANO = " & ANO_ATUAL & " AND COD_PERFIL = 3 AND LINHA_PRODUTO = '" & LINHA_PRODUTO & "' AND EMAIL = '" & cmb_Usuario.Text & "')"
        ...
        If dtr.HasRows Then
            ...
            If Not (IsDBNull(dtr("TOTAL"))) Then TOTAL_COTA = dtr("TOTAL")
            If Not (IsDBNull(dtr("MEDIA"))) Then MEDIA_COTA = dtr("MEDIA")
            If Not (IsDBNull(dtr("YTD"))) Then YTD_COTA = dtr("YTD")
            If Not (IsDBNull(dtr("TRI01"))) Then TRI01_COTA = dtr("TRI01")
            If Not (IsDBNull(dtr("TRI02"))) Then TRI02_COTA = dtr("TRI02")
            If Not (IsDBNull(dtr("TRI03"))) Then TRI03_COTA = dtr("TRI03")
            If Not (IsDBNull(dtr("TRI04"))) Then TRI04_COTA = dtr("TRI04")
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_COTA, 0)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
    ...
    Private Sub Linha_Performance_Atual(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        If JAN_COTA <> 0 Then JAN_PERFORMANCE = Round((JAN_DEMANDA_ATUAL / JAN_COTA) * 100, 2)
        If FEV_COTA <> 0 Then FEV_PERFORMANCE = Round((FEV_DEMANDA_ATUAL / FEV_COTA) * 100, 2)
        If MAR_COTA <> 0 Then MAR_PERFORMANCE = Round((MAR_DEMANDA_ATUAL / MAR_COTA) * 100, 2)
        If ABR_COTA <> 0 Then ABR_PERFORMANCE = Round((ABR_DEMANDA_ATUAL / ABR_COTA) * 100, 2)
        If MAI_COTA <> 0 Then MAI_PERFORMANCE = Round((MAI_DEMANDA_ATUAL / MAI_COTA) * 100, 2)
        If JUN_COTA <> 0 Then JUN_PERFORMANCE = Round((JUN_DEMANDA_ATUAL / JUN_COTA) * 100, 2)
        If JUL_COTA <> 0 Then JUL_PERFORMANCE = Round((JUL_DEMANDA_ATUAL / JUL_COTA) * 100, 2)
        If AGO_COTA <> 0 Then AGO_PERFORMANCE = Round((AGO_DEMANDA_ATUAL / AGO_COTA) * 100, 2)
        If SET_COTA <> 0 Then SET_PERFORMANCE = Round((SET_DEMANDA_ATUAL / SET_COTA) * 100, 2)
        If OUT_COTA <> 0 Then OUT_PERFORMANCE = Round((OUT_DEMANDA_ATUAL / OUT_COTA) * 100, 2)
        If NOV_COTA <> 0 Then NOV_PERFORMANCE = Round((NOV_DEMANDA_ATUAL / NOV_COTA) * 100, 2)
        If DEZ_COTA <> 0 Then DEZ_PERFORMANCE = Round((DEZ_DEMANDA_ATUAL / DEZ_COTA) * 100, 2)
        If TRI01_COTA <> 0 Then TRI01_PERFORMANCE = Round((TRI01_DEMANDA_ATUAL / TRI01_COTA) * 100, 2)
        If TRI02_COTA <> 0 Then TRI02_PERFORMANCE = Round((TRI02_DEMANDA_ATUAL / TRI02_COTA) * 100, 2)
        If TRI03_COTA <> 0 Then TRI03_PERFORMANCE = Round((TRI03_DEMANDA_ATUAL / TRI03_COTA) * 100, 2)
        If TRI04_COTA <> 0 Then TRI04_PERFORMANCE = Round((TRI04_DEMANDA_ATUAL / TRI04_COTA) * 100, 2)
        If YTD_COTA <> 0 Then YTD_PERFORMANCE = Round((YTD_DEMANDA_ATUAL / YTD_COTA) * 100, 2)
        If TOTAL_COTA <> 0 Then TOTAL_PERFORMANCE = Round((TOTAL_DEMANDA_ATUAL / TOTAL_COTA) * 100, 2)
        If MEDIA_COTA <> 0 Then MEDIA_PERFORMANCE = Round((MEDIA_DEMANDA_ATUAL / MEDIA_COTA) * 100, 2)
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(MEDIA_PERFORMANCE, 2) & " %"
        If TOTAL_PERFORMANCE < 80 Then cel.BackColor = Drawing.Color.FromArgb(255, 100, 100)
        If TOTAL_PERFORMANCE >= 95 Then cel.BackColor = Drawing.Color.FromArgb(100, 255, 100)
        If TOTAL_PERFORMANCE >= 105 Then cel.BackColor = Drawing.Color.LightSkyBlue
        If TOTAL_PERFORMANCE = 0 Then cel.BackColor = Drawing.Color.White
