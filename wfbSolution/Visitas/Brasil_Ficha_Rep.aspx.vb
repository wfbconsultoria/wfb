Imports System.Math
Partial Class Brasil_Ficha_Rep
    Inherits System.Web.UI.Page
    Dim M As New clsMaster
    Dim Pagina As String = "Brasil_Ficha.aspx"
        ...
        If dtr_linha_produto.HasRows Then
            ...
            Do While dtr_linha_produto.Read
                ...
                If COD_GRUPO_BOMBA <> 0 Then
                    ...
                    Linha_Bombas_Representante(dtr_linha_produto("LINHA"), "Bombas Rep - Alocadas " & ANO_ATUAL, "", True)
                ...
                sql_rep = "Select Distinct EMAIL_REPRESENTANTE, REPRESENTANTE FROM VIEW_DEMANDA_001_DETALHADO WHERE ANO = " & ANO_ATUAL & " ORDER BY REPRESENTANTE"
                cmd_rep.Connection = cnn_rep
                cmd_rep.CommandText = sql_rep
                dtr_rep = cmd_rep.ExecuteReader
                Linha_Vazia(30, "", True)

                Linha_Titulo(dtr_linha_produto("LINHA") & " - " & "Representantes")
                While dtr_rep.Read()
                    ...
                    Linha_Cota_Atual_REP(dtr_linha_produto("LINHA"), "Cota", True)
                    Linha_Demanda_Atual_REP(dtr_linha_produto("LINHA"), "Demanda", True)
                    Linha_Performance_Atual_REP(dtr_linha_produto("LINHA"), "Performance (%)", True)
                    If COD_GRUPO_BOMBA <> 0 Then
                        'Produtividade de Bombas Representante
                        Linha_Bombas_Representante(dtr_linha_produto("LINHA"), "Bombas Rep - Alocadas " & ANO_ATUAL, EMAIL, True)
                        Linha_Bombas_Produtividade_REP(dtr_linha_produto("LINHA"), "Produtividade " & ANO_ATUAL, True)
                    End If
    ...
    End Sub
    Private Sub Linha_Bombas_Representante(LINHA_PRODUTO As String, TITULO_LINHA As String, EMAIL_REPRESENTANTE As String, Exibir As Boolean)
        If COD_GRUPO_BOMBA = 0 Then Exit Sub
        'RECUPERA INFORMAÇÕES
        JAN_BOMBAS_REP = 0
        FEV_BOMBAS_REP = 0
        ...
        If EMAIL_REPRESENTANTE <> "" Then
            sql = sql & "AND (EMAIL_REPRESENTANTE = '" & EMAIL_REPRESENTANTE & "')"
        End If
    ...
    Private Sub Linha_Bombas_Produtividade_REP(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        cel = New TableCell
        rw.Cells.Add(cel)
        cel.Text = FormatNumber(SET_BOMBAS_PRODUTIVIDADE_REP, 2)
        cel.BorderStyle = BorderStyle.Solid
        cel.BorderColor = Drawing.Color.Silver
        cel.BorderWidth = 1
