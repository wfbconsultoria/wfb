...
Partial Class Estabelecimento_Incluir_RF
    Inherits System.Web.UI.Page
    Public jscript As String
    'Váriaveis Padrão
    Dim M As New clsMaster
    Dim Pagina As String = "Estabelecimento_Incluir_RF.aspx"
    Dim Titulo As String = "Inclusão de Cliente - " & ConfigurationManager.AppSettings("NOME_SISTEMA").ToString
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ' Verifica o perfil de acesso para carregar o combo de representantes para setorização
        sql = sql & " Select EMAIL, NOME From TBL_USUARIOS Where (COD_PERFIL = '4') And (ATIVO = 'True') And ORFAO = 'True' "
        If Session("COD_PERFIL_LOGIN") = "0" Then sql = sql
        If Session("COD_PERFIL_LOGIN") = "1" Then sql = sql & " And (DIRETOR = '" & Session("EMAIL_LOGIN") & "') And ORFAO = 'True'"
        If Session("COD_PERFIL_LOGIN") = "2" Then sql = sql & " And (GERENTE = '" & Session("EMAIL_LOGIN") & "') And ORFAO = 'True'"
        If Session("COD_PERFIL_LOGIN") = "3" Then sql = sql & " And (COORDENADOR = '" & Session("EMAIL_LOGIN") & "') And ORFAO = 'True'"
        ...
        'variaveis de conexão e acesso ao banco de dados
        ...
        If dtr.HasRows Then
            ...
            If Not IsDBNull(dtr("ESTABELECIMENTO")) Then ESTABELECIMENTO = M.ConverteTexto(dtr("ESTABELECIMENTO"))
            If Not IsDBNull(dtr("ESTABELECIMENTO_CNPJ")) Then ESTABELECIMENTO_CNPJ = M.ConverteTexto(dtr("ESTABELECIMENTO_CNPJ"))
            If Not IsDBNull(dtr("RAZAO_SOCIAL")) Then RAZAO_SOCIAL = M.ConverteTexto(dtr("RAZAO_SOCIAL"))
            If Not IsDBNull(dtr("NOME_FANTASIA")) Then NOME_FANTASIA = M.ConverteTexto(dtr("NOME_FANTASIA"))
        ...
        Else
            Alert("Estabelecimento não encontrado!", True, "Estabelecimentos_Pesquisa_RF.aspx")
        End If
    ...
    Protected Sub btn_Gravar_Click(sender As Object, e As EventArgs) Handles cmd_Gravar.Click
        ...
        If Request.QueryString("INCLUSO_CLIENTE") = False Then
            sql = ""
            sql = sql & " Insert Into TBL_ESTABELECIMENTOS_RF "
            ...
            If M.ExecutarSQL(sql) = True Then
                ...
                sql = sql & " (CNPJ, ESTABELECIMENTO, RAZAO_SOCIAL,"
                sql = sql & " ENDERECO, COMPLEMENTO, BAIRRO, CEP, COD_MUNICIPIO,"
                sql = sql & " COD_ESFERA_ADMINISTRATIVA, "
                sql = sql & " INCLUSAO_DATA, INCLUSAO_EMAIL)"
                sql = sql & " Values ("
                sql = sql & M.Converte_Valor(CNPJ) & ", "
                ...
                sql = sql & "'" & Session("EMAIL_LOGIN") & "') "
                If M.ExecutarSQL(sql) = True Then
                    'VERIFICA SE JA ESTA SETORIZADO PARA A PESSOA
                    sql = ""
                    sql = "Select *  From TBL_SETORIZACAO Where CNPJ = " & M.Converte_Valor(CNPJ)
                    cmd.Connection = cnn
                    cmd.CommandText = sql
                    dtr = cmd.ExecuteReader
                    If dtr.HasRows Then
                        Alert("Este estabelecimento já está setorizado!", True, "Estabelecimentos_Pesquisa_RF.aspx")
                    Else
                        ...
                        If M.ExecutarSQL(sql) = True Then
                            ...
                            M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "SETORIZOU ESTABELECIMENTO", "CNPJ: " & CNPJ)
                            Alert("Estabelecimento incluido e setorizado com  sucesso!", True, "Estabelecimentos_Pesquisa_RF.aspx")
                        Else
                            ...
                            Alert("Estabelecimento incluido, mas não setorizado! O Erro foi enviado ao suporte e será analisado!", True, "Estabelecimentos_Pesquisa_RF.aspx")
                        End If
                        Exit Sub
                    End If
                    dtr.Close()
                ...
                Else
                    ...
                    M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "ERRO EM INCLUIR ESTABELECIMENTO", "CNPJ: " & Session("CNPJ") & " PARA: " & cmb_USUARIOS.Text)
                    Mensagem = "ERRO  - INCLUSÃO DE ESTABELECIMENTOS" & Chr(10)
                    Mensagem = Mensagem & "CNPJ: " & Session("CNPJ") & Chr(10)
                    Mensagem = Mensagem & "Para: " & cmb_USUARIOS.Text & Chr(10)
                    Mensagem = Mensagem & "SQL: " & sql & Chr(10)
                    M.EnviarEmail("ERRO  - INCLUSÃO DE ESTABELECIMENTOS", Mensagem, True, False, "", "")
                    'Alert("Erro ao incluir Cliente! Um e-mail foi enviado ao suporte para verficação.", True, "Estabelecimentos_Pesquisa_RF.aspx")
                End If
            Else
        ...
        Else
            ...
            Else
                ...
                If M.ExecutarSQL(sql) = True Then
                    ...
                    M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "SETORIZOU ESTABELECIMENTO", "CNPJ: " & Session("CNPJ") & " - " & " PARA: " & cmb_USUARIOS.Text)
                    M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "SETORIZOU ESTABELECIMENTO", "CNPJ: " & CNPJ)
                    Alert("Estabelecimento foi setorizado com sucesso!", True, "Estabelecimentos_Pesquisa_RF.aspx")
                Else
                    M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "ERRO EM SETORIZAR ESTABELECIMENTO", "CNPJ: " & Session("CNPJ") & " PARA: " & cmb_USUARIOS.Text)
                    Mensagem = "ERRO  - SETORIZACAO DE ESTABELECIMENTOS" & Chr(10)
                    Mensagem = Mensagem & "CNPJ: " & Session("CNPJ") & Chr(10)
