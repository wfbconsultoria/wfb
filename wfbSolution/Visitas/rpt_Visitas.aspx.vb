...
Partial Class rpt_Visitas
    ...
    Protected Sub cmb_Ano_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_Ano.SelectedIndexChanged
        ...
        If Session("COD_PERFIL_LOGIN") = 0 Then Sql = "Select * From VIEW_VISITAS_GERAL WHERE MES_VISITA_SIGLA = '" & "' AND ANO_VISITA = '" & cmb_Ano.Text & "'"
        If Session("COD_PERFIL_LOGIN") = 1 Then Sql = "Select * From VIEW_VISITAS_GERAL WHERE MES_VISITA_SIGLA = '" & "' AND ANO_VISITA = '" & cmb_Ano.Text & "' AND EMAIL_DIRETOR = '" & Session("EMAIL_LOGIN") & "'"
        If Session("COD_PERFIL_LOGIN") = 2 Then Sql = "Select * From VIEW_VISITAS_GERAL WHERE MES_VISITA_SIGLA = '" & "' AND ANO_VISITA = '" & cmb_Ano.Text & "' AND EMAIL_GERENTE = '" & Session("EMAIL_LOGIN") & "'"
        If Session("COD_PERFIL_LOGIN") = 3 Then Sql = "Select * From VIEW_VISITAS_GERAL WHERE MES_VISITA_SIGLA = '" & "' AND ANO_VISITA = '" & cmb_Ano.Text & "' AND EMAIL_GERENTE_CONTA = '" & Session("EMAIL_LOGIN") & "'"
        If Session("COD_PERFIL_LOGIN") = 4 Then Sql = "Select * From VIEW_VISITAS_GERAL WHERE MES_VISITA_SIGLA = '" & "' AND ANO_VISITA = '" & cmb_Ano.Text & "' AND EMAIL_VISITANTE = '" & Session("EMAIL_LOGIN") & "'"
