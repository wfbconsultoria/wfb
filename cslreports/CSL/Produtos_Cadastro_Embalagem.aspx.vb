...
Partial Class Produtos_Cadastro_Embalagem
    Inherits System.Web.UI.Page
    'Váriaveis Padrão
    Dim M As New clsMaster
    Dim Pagina As String = "Produtos_Cadastro_Embalagem.aspx"
    Dim Titulo As String = "Embalagens de Produto - " & ConfigurationManager.AppSettings("NOME_SISTEMA").ToString
    ...
    Protected Sub cmd_Gravar_Click(sender As Object, e As EventArgs) Handles cmd_Gravar.Click
        If Len(txt_Embalagem.Text) = 0 Then
            Alert("Digite a embalagem de produto antes de gravar!", False, "")
            Exit Sub
        End If
        Sql = ""
        Sql = "Insert Into TBL_PRODUTOS_EMBALAGEM (PRODUTO_EMBALAGEM_VENDA, INCLUSAO_EMAIL, INCLUSAO_DATA) Values "
        Sql = Sql & "('" & M.ConverteTexto(txt_Embalagem.Text) & "', "
        ...
        If M.ExecutarSQL(Sql) = True Then
            ...
            dts_Lista.DataBind()
            gdv_Lista.DataBind()
            M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "Inclui embalagem de produto", txt_Embalagem.Text)
            Alert("Embalagem de Produto incluida com sucesso!", False, "")
        Else
            Mensagem = "ERRO - INCLUSAO EMBALAGEM DE PRODUTO - " & Pagina & " - " & Now() & Chr(10)
            Mensagem = Mensagem & "EMBALAGEM DE PRODUTO: " & txt_Embalagem.Text & Chr(10)
            Mensagem = Mensagem & "Por: " & Session("USUARIO_LOGIN") & " - " & Session("EMAIL_LOGIN")
            M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "ERRO", txt_Embalagem.Text)
            M.EnviarEmail("ERRO - INCLUSAO EMBALAGEM DE PRODUTO", Mensagem, True, False, "", "")
    ...
    Protected Sub gdv_Lista_RowUpdated(sender As Object, e As GridViewUpdatedEventArgs) Handles gdv_Lista.RowUpdated
        M.Inclui_Alteracao(gdv_Lista.SelectedValue, Session("EMAIL_LOGIN"), "COD_PRODUTO_EMBALAGEM_VENDA", "TBL_PRODUTOS_EMBALAGEM")
    End Sub
    Protected Sub gdv_Lista_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gdv_Lista.SelectedIndexChanged
        ...
        QTD_REGISTROS = M.Consiste_Exclusao(gdv_Lista.SelectedValue, "COD_PRODUTO_EMBALAGEM_VENDA", "TBL_PRODUTOS")
        If QTD_REGISTROS > 0 Then
            Alert("Esta embalagem não pode ser excluido pois existem " & QTD_REGISTROS & " produtos relacionados", False, "")
        Else
            If M.ExecutarSQL("Delete From TBL_PRODUTOS_EMBALAGEM Where COD_PRODUTO_EMBALAGEM_VENDA = " & gdv_Lista.SelectedValue) = True Then
                M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "Excluiu Embalagem de produto COD", gdv_Lista.SelectedValue)
                Alert("Embalagem excluida com sucesso ", False, "")
                dts_Lista.DataBind()
                gdv_Lista.DataBind()
            End If
        End If
