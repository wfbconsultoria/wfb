...
Partial Class Estabelecimentos_Grupos
    Inherits System.Web.UI.Page
    'Váriaveis Padrão
    Dim M As New clsMaster
    Dim Pagina As String = "Estabelecimentos_Grupos.aspx"
    Dim Titulo As String = "Grupos de Clientes - " & ConfigurationManager.AppSettings("NOME_SISTEMA").ToString
    ...
    Protected Sub cmd_Gravar_Click(sender As Object, e As EventArgs) Handles cmd_Gravar.Click
        If Len(txt_Grupo.Text) = 0 Then
            Alert("Digite o Grupo de Estabelecimento antes de gravar!", False, "")
            Exit Sub
        End If
        Sql = ""
        Sql = "Insert Into TBL_ESTABELECIMENTOS_GRUPOS (GRUPO_ESTABELECIMENTO) Values ('" & M.ConverteTexto(txt_Grupo.Text) & "')"
        ...
        Else
            Mensagem = "ERRO - INCLUSAO GRUPO ESTABELECIMENTO - " & Pagina & " - " & Now() & Chr(10)
            Mensagem = Mensagem & "Grupo Estabelecimento: " & txt_Grupo.Text & Chr(10)
            ...
            M.EnviarEmail("ERRO - INCLUSAO GRUPO ESTABELECIMENTO", Mensagem, True, False, "", "")
    ...
    Protected Sub gdv_Lista_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gdv_Lista.SelectedIndexChanged
        ...
        QTD_REGISTROS = M.Consiste_Exclusao(gdv_Lista.SelectedValue, "ID_GRUPO_ESTABELECIMENTO", "TBL_ESTABELECIMENTOS")
        If QTD_REGISTROS > 0 Then
            Alert("Este grupo não pode ser excluido pois existem " & QTD_REGISTROS & " clientes relacionados", False, "")
        Else
            If M.ExecutarSQL("Delete From TBL_ESTABELECIMENTOS_GRUPOS Where ID_GRUPO_ESTABELECIMENTO = " & gdv_Lista.SelectedValue) = True Then
                M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "Excluiu grupo de estabelecimento ID", gdv_Lista.SelectedValue)
