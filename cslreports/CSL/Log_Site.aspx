...
<div id ="Corpo">
    ...
    <asp:GridView ID="gdv_Log_Site" runat="server" AutoGenerateColumns="False" DataSourceID="dts_VIEW_LOG_SITE" Width="100%" AllowSorting="True">
        ...
        SelectCommand="SELECT top(5000) * FROM [VIEW_LOG_SITE] WHERE [SISTEMA] = @SISTEMA AND [IP] &lt;&gt; @IP ORDER BY [DATA] DESC, [HORA] DESC, [MINUTO] DESC">
            <SelectParameters>
                <asp:Parameter DefaultValue="False" Name="SISTEMA" Type="Boolean" />
                <asp:Parameter DefaultValue="189.111.216.129" Name="IP" Type="String" />
            </SelectParameters>
    </asp:SqlDataSource>
    ...
    <asp:SqlDataSource ID="dts_Usuarios" runat="server" ConnectionString="<%$ ConnectionStrings:cnnStr %>" SelectCommand="Select '@' as EMAIL, '#Todos' as NOME UNION ALL SELECT EMAIL,NOME FROM [VIEW_USUARIOS] WHERE (([SISTEMA] = @SISTEMA) AND ([BLOQUEADO] = @BLOQUEADO)) ORDER BY [NOME]">
        <SelectParameters>
            <asp:Parameter DefaultValue="False" Name="SISTEMA" Type="Boolean" />
