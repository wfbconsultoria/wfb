...
Partial Class Distribuidores_Grupos
    Inherits System.Web.UI.Page
    'Váriaveis Padrão
    Dim M As New clsMaster
    Dim Pagina As String = "Distribuidores_Grupos.aspx"
    Dim Titulo As String = "Grupos de Distribuidores - " & ConfigurationManager.AppSettings("NOME_SISTEMA").ToString
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("SISTEMA") = False Then Acesso = True 'Administrador
    ...
    Protected Sub cmd_Gravar_Click(sender As Object, e As EventArgs) Handles cmd_Gravar.Click
        If Len(txt_Grupo.Text) = 0 Then
            Alert("Digite o Grupo de Distribuidor antes de gravar!", False, "")
            Exit Sub
        End If
        Sql = ""
        Sql = "Insert Into TBL_DISTRIBUIDORES_GRUPOS (GRUPO_DISTRIBUIDOR) Values ('" & M.ConverteTexto(txt_Grupo.Text) & "')"

        If M.ExecutarSQL(Sql) = True Then
            txt_Grupo.Text = ""
            dts_Lista.DataBind()
            gdv_Lista.DataBind()
            M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "Inclui grupo de distribuidor", txt_Grupo.Text)
            Alert("Grupo de distribuidor incluido com sucesso!", False, "")
        Else
            Mensagem = "ERRO - INCLUSAO GRUPO distribuidor - " & Pagina & " - " & Now() & Chr(10)
            Mensagem = Mensagem & "Grupo distribuidor: " & txt_Grupo.Text & Chr(10)
            ...
            M.EnviarEmail("ERRO - INCLUSAO GRUPO distribuidor", Mensagem, True, False, "", "")
    ...
    Protected Sub gdv_Lista_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gdv_Lista.SelectedIndexChanged
        ...
        QTD_REGISTROS = M.Consiste_Exclusao(gdv_Lista.SelectedValue, "ID_GRUPO_DISTRIBUIDOR", "TBL_DISTRIBUIDORES")
        If QTD_REGISTROS > 0 Then
            Alert("Este grupo não pode ser excluido pois existem " & QTD_REGISTROS & " distribuidores relacionados", False, "")
        Else
            If M.ExecutarSQL("Delete From TBL_DISTRIBUIDORES_GRUPOS Where ID_GRUPO_DISTRIBUIDOR = " & gdv_Lista.SelectedValue) = True Then
                M.LogPagina(Session("SESSAO_LOGIN"), Session("IP_LOGIN"), Session("EMAIL_LOGIN"), Pagina, "Excluiu grupo de DISTRIBUIDOR ID", gdv_Lista.SelectedValue)
