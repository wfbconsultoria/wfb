...
Err_AbreTabela:
        ...
        Return False
    End Function
    'Executar Select na base 
    Public Function ExecutarSelect(ByVal SQL As String) As System.Data.SqlClient.SqlDataReader
        On Error GoTo Err_ExcutarSelect

        ExecutarSelect = Nothing
        Dim cn As New System.Data.SqlClient.SqlConnection
        Dim cmd As System.Data.SqlClient.SqlCommand = cn.CreateCommand
        cn.ConnectionString = ConfigurationManager.ConnectionStrings("cnnStr").ConnectionString
        'verifica se há uma conexão aberta
        ...
        cmd.Connection = cn
        cmd.CommandText = SQL
        ExecutarSelect = cmd.ExecuteReader
        Exit Function
...
Err_ExcutarSelect:
    ...
    Public Function SQL_PADRAO(Tipo As String) As String
        SQL_PADRAO = ""
        Select Tipo
            Case Is = "VIEW_ESTABELECIMENTOS_001_DETALHADO"
                SQL_PADRAO = SQL_PADRAO & " Select Distinct "
                SQL_PADRAO = SQL_PADRAO & " CNPJ, CNPJ_FORMATADO, CNES, COD_INTERNO, "
                SQL_PADRAO = SQL_PADRAO & " GRUPO_ESTABELECIMENTO, APELIDO, ESTABELECIMENTO, ESTABELECIMENTO_CNPJ, RAZAO_SOCIAL, NOME_FANTASIA, "
                SQL_PADRAO = SQL_PADRAO & " ENDERECO, LOGRADOURO, NUMERO, COMPLEMENTO, BAIRRO, CEP, COD_MUNICIPIO, MUNICIPIO, UF, ESTADO, REGIAO, REGIAO_SAUDE, MICRO_REGIAO_SAUDE, "
                SQL_PADRAO = SQL_PADRAO & " TIPO_PESSOA, COD_ESFERA_ADMINISTRATIVA, ESFERA, GESTAO, COD_NATUREZA_JURIDICA, NATUREZA_JURIDICA_DESCRICAO, COD_CNAE, CNAE_DESCRICAO, "
                SQL_PADRAO = SQL_PADRAO & " DATA_FUNDACAO, DATA_SITUACAORFB, SITUACAORFB, INCLUSAO, ULTIMA_ATUALIZACAO_RECEITA, "
                SQL_PADRAO = SQL_PADRAO & " SETORIZADO, QTD_SETORIZACOES, SETORIZADO_VALOR, GERENTE_CONTA, EMAIL_GERENTE_CONTA "
                SQL_PADRAO = SQL_PADRAO & " FROM VIEW_ESTABELECIMENTOS_001_DETALHADO"

            Case Is = "VIEW_ESTABELECIMENTOS_001_DETALHADO_SETORIZADO"
                SQL_PADRAO = SQL_PADRAO & " Select Distinct "
                SQL_PADRAO = SQL_PADRAO & " CNPJ, CNPJ_FORMATADO, CNES, COD_INTERNO, "
                SQL_PADRAO = SQL_PADRAO & " GRUPO_ESTABELECIMENTO, APELIDO, ESTABELECIMENTO, ESTABELECIMENTO_CNPJ, RAZAO_SOCIAL, NOME_FANTASIA, "
                SQL_PADRAO = SQL_PADRAO & " ENDERECO, LOGRADOURO, NUMERO, COMPLEMENTO, BAIRRO, CEP, COD_MUNICIPIO, MUNICIPIO, UF, ESTADO, REGIAO, REGIAO_SAUDE, MICRO_REGIAO_SAUDE, "
                ...
                SQL_PADRAO = SQL_PADRAO & " DATA_FUNDACAO, DATA_SITUACAORFB, SITUACAORFB, INCLUSAO, ULTIMA_ATUALIZACAO_RECEITA, "
                SQL_PADRAO = SQL_PADRAO & " SETORIZADO, QTD_SETORIZACOES, SETORIZADO_VALOR, GERENTE_CONTA, EMAIL_GERENTE_CONTA "
                SQL_PADRAO = SQL_PADRAO & " FROM VIEW_ESTABELECIMENTOS_001_DETALHADO_SETORIZADO "

            Case Is = "VIEW_ESTABELECIMENTOS_001_DETALHADO_ORFAOS"
                SQL_PADRAO = SQL_PADRAO & " Select * From VIEW_ESTABELECIMENTOS_001_DETALHADO_ORFAOS"
        End Select
    End Function
    Public Function Inclui_Alteracao(Parametro As String, EMAIL As String, Coluna As String, Tabela As String) As Double
        'Verifica se um determinado registro em uma determinada tabela pode ser excluido
        On Error GoTo Err_Inclui_Alteracao
        ...
        sql = ""
        sql = "Update [" & Tabela & "] set ALTERACAO_EMAIL = '" & EMAIL & "' , ALTERACAO_DATA = " & RecuperaData() & " WHERE [" & Coluna & "] = '" & Parametro & "'"

        If ExecutarSQL(sql) = True Then
            Inclui_Alteracao = 1
        Else
            Write(sql)
        End If
...
Err_Consiste_Exclusao:
    ...
    'Função para ler uma string a partir de um parametro ------------------------------------------
    Public Function fromParameter(ByVal campo As String, ByVal parametro As String) As String
        Dim chars() = campo.ToCharArray() 'Transformando o campo em array
        Dim find As Boolean = False
        campo = "" 'Esvaziando email'

        'Lendo campos do array
        For cont = 0 To chars.Length - 1
            If chars(cont) = parametro Then
                find = True 'Validando se realmente é um email'
            End If
            If find = True Then
                campo += chars(cont) 'Armazenando dominio do email'
            End If
        Next
