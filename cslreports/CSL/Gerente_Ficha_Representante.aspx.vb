Imports System.Math
Partial Class Gerente_Ficha_Representante
    Inherits System.Web.UI.Page
    Dim M As New clsMaster
    Dim Pagina As String = "Gerente_Ficha_Representante.aspx"
    ...
    Dim cnn_rep As New System.Data.SqlClient.SqlConnection
    Dim cmd_rep As New System.Data.SqlClient.SqlCommand
    Dim dtr_rep As System.Data.SqlClient.SqlDataReader
    Dim sql_rep As String
    ...
    Protected Sub cmb_Produto_Linha_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_Produto_Linha.SelectedIndexChanged
        Response.Redirect("Gerente_Ficha_Representante.aspx?TIPO_MOVIMENTO=" & cmb_TIPO_MOVIMENTO.Text & "&GERENTE=" & cmb_EMAIL_GERENTE.Text & "&LINHA=" & cmb_Produto_Linha.Text & "&ANO=" & cmb_Ano.Text)
    End Sub
    ...
    Protected Sub Atualiza_Relatorio()
        ...
        cnn.Open()
        cnn_rep.ConnectionString = ConfigurationManager.ConnectionStrings("cnnStr").ConnectionString
        cnn_rep.Open()
        ...
        If dtr_linha_produto.HasRows Then
            ...
            Do While dtr_linha_produto.Read
                ...
                sql_rep = "Select Distinct EMAIL_REPRESENTANTE, REPRESENTANTE FROM VIEW_DEMANDA_001_DETALHADO WHERE ANO = " & ANO_ATUAL & " AND EMAIL_GERENTE = '" & cmb_EMAIL_GERENTE.Text & "' ORDER BY REPRESENTANTE"
                cmd_rep.Connection = cnn_rep
                cmd_rep.CommandText = sql_rep
                dtr_rep = cmd_rep.ExecuteReader
                Linha_Vazia(30, "", True)

                Linha_Titulo(dtr_linha_produto("PRODUTO_LINHA") & " - " & "Representantes")
                While dtr_rep.Read()

                    EMAIL = dtr_rep("EMAIL_REPRESENTANTE")
                    NOME = dtr_rep("REPRESENTANTE")
                    Linha_Vazia(5, "", True)
                    Linha_Cabecalho_Meses(NOME)
                    Linha_Cota_Atual_REP(dtr_linha_produto("PRODUTO_LINHA"), "Cota", True)
                    Linha_Demanda_Atual_REP(dtr_linha_produto("PRODUTO_LINHA"), "Demanda", True)
                    Linha_Performance_Atual_REP(dtr_linha_produto("PRODUTO_LINHA"), "Performance (%)", True)
    ...
    Private Sub Linha_Vazia(Altura As Integer, Titulo As String, Negrito As Boolean)
        ...
        cel.BorderStyle = BorderStyle.None
    End Sub
    Private Sub Linha_Demanda_Atual_REP(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        ...
        sql = sql & "SUM(TOTAL) AS TOTAL, SUM(TRI01) AS TRI01, SUM(TRI02) AS TRI02,SUM(TRI03) AS TRI03,SUM(TRI04) AS TRI04,SUM(YTD) AS YTD "
        sql = sql & "FROM dbo.VIEW_DEMANDA_001_DETALHADO "
        sql = sql & "WHERE (ANO = " & ANO_ATUAL & " AND EMAIL_REPRESENTANTE = '" & EMAIL & "' AND PRODUTO_LINHA = '" & LINHA_PRODUTO & "')"
    ...
    End Sub
    Private Sub Linha_Cota_Atual_REP(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        'RECUPERA INFORMAÇÕES
        JAN_COTA = 0
        FEV_COTA = 0
        MAR_COTA = 0
        ...
        sql = sql & "FROM dbo.VIEW_COTA_DEMANDA_DETALHADO_UNIVERSAL_UN_EQ_DIST_GERAL "
        sql = sql & "WHERE (ANO = " & ANO_ATUAL & " AND EMAIL_REPRESENTANTE = '" & EMAIL & "' AND PRODUTO_LINHA = '" & LINHA_PRODUTO & "') "
    ...
    End Sub
    Private Sub Linha_Performance_Atual_REP(LINHA_PRODUTO As String, TITULO_LINHA As String, Exibir As Boolean)
        JAN_PERFORMANCE = 0
        FEV_PERFORMANCE = 0
        MAR_PERFORMANCE = 0
        ABR_PERFORMANCE = 0
    ...
    'FUNCOES PARA EXIBIÇÃO DE OPORTUNIDADES COMENTADAS
    ...
    '    sql = sql & "WHERE (ANO_OPORTUNIDADE = " & ANO_ATUAL & ") AND (PRODUTO_LINHA = '" & LINHA_PRODUTO & "')"
